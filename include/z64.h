...
typedef struct {
    ...
    /* 0x00008 */ Gfx polyOpaBuffer[0x2000];
...
typedef enum { MESSAGE_ICON_TRIANGLE, MESSAGE_ICON_SQUARE, MESSAGE_ICON_ARROW } MessageBoxIcon;
...
typedef enum {
    ...
    /* 0x06 */ DO_ACT_DECIDE,
    /* 0x07 */ DO_ACT_DIVE,
    /* 0x08 */ DO_ACT_FASTER, // changed to "Whip" in Spaceworld
    /* 0x09 */ DO_ACT_THROW,
    /* 0x0A */ DO_ACT_NAVI,
    /* 0x0B */ DO_ACT_CLIMB,
    /* 0x0C */ DO_ACT_DROP,
    ...
    /* 0x1C */ DO_ACT_NUM_8,
    /* 0x1D */ DO_ACT_MAX,
    /* 0x27 */ DO_ACT_UNK_27 = 0x27,
    /* 0x44 */ DO_ACT_UNK_44 = 0x44,
    // Added actions for SpaceWorld (make sure to update `swActionTextures`)
    SW_DO_ACT_RESET = 1000,
    SW_DO_ACT_RIDE,
    SW_DO_ACT_VIEW,
    SW_DO_ACT_RETURN
} DoAction;
...
#define PAUSE_ITEM_MAP_SCREEN 998
...
typedef void (*FileSelectUpdateFunc)(struct FileChooseContext*);
typedef void (*FileSelectDrawFunc)(struct FileChooseContext*);
...
typedef struct FileChooseContext {
    ...
    s16 buttonIndex;
    s16 staticLabelsAlpha; // "select file" and controls label
    s16 inputTimerX;
    s16 inputTimerY;
    s16 xIndexOffset;
    s16 yIndexOffset;
    s16 stickRelX;
    s16 stickRelY;
    FileSelectUpdateFunc update;
    FileSelectDrawFunc draw;
    s16 cursorCol[3];
    s16 panelCol[3];
    s16 screenFillAlpha;
    u8 cursorDir;
    ...
    s16 creditsPage;
    s16 creditsTimer;
    f32 creditsAlphaFrac; // multiplied with creditsAlpha
    s16 alphaMode;
    f32 effectAlpha;
    f32 titleAlpha;
    s16 logoAlpha;
...
struct LZODecode {
    u8* buf[650 * 1024]; // buffer which stores the entirety of the compressed data
};
