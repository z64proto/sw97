...
s32 gSkipN64Logo = false;
s32 gSkipTitle = false;
...
void Gameplay_Init(GameState* thisx) {
    ...
    // Modified failsafe to prevent crash on invalid entrance
    if (GET_ENTRANCE_SCENE(gSaveContext.entranceIndex) != SCENE_SPOT04_OLD &&
        (GET_ENTRANCE_SCENE(gSaveContext.entranceIndex) < SCENE_FSTDAN ||
         GET_ENTRANCE_SCENE(gSaveContext.entranceIndex) >= SCENE_SPACEWORLD_MAX)) {
    ...
    SystemArena_Display();
    GameState_Realloc(&globalCtx->state, 0x3D4790); // originally only 0x1D4790
    ...
    Gameplay_SpawnScene(globalCtx, GET_ENTRANCE_SCENE(gSaveContext.entranceIndex),
                        GET_ENTRANCE_SPAWN(gSaveContext.entranceIndex));
    ...
    if (gSaveContext.gameMode != 1) {
        if (gSaveContext.nextTransition == 0xFF) {
            globalCtx->fadeTransition = GET_ENTRANCE_TRANSITION(gSaveContext.entranceIndex);
        } else {
...
void Gameplay_Update(GlobalContext* globalCtx) {
    ...
    if ((globalCtx->sceneNum == SCENE_HYRULE_FIELD) && (gSaveContext.sceneSetupIndex == 1) &&
        (globalCtx->csCtx.state != CS_STATE_IDLE)) {
        Input* input = &globalCtx->state.input[0];

        // Force the opening cutscene to use the letterbox
        Interface_ChangeAlpha(1);
        ShrinkWindow_SetVal(0x20);
        ShrinkWindow_SetCurrentVal(0x20);

        if ((globalCtx->csCtx.frames > 5 && globalCtx->csCtx.frames < 1480 &&
             (CHECK_BTN_ALL(input->press.button, BTN_START) || CHECK_BTN_ALL(input->press.button, BTN_A) ||
              CHECK_BTN_ALL(input->press.button, BTN_B))) ||
            globalCtx->csCtx.frames >= 1480) {

            // Load the file select from the opening cutscene
            ...
            switch (globalCtx->transitionMode) {
                case 1:
                    if (globalCtx->sceneLoadFlag != -0x14) {
                        ...
                        if (!(GET_ENTRANCE_MUSIC_FLAG(globalCtx->nextEntranceIndex))) { // Continue BGM Off
...
skip:
    ...
    {
        f32 oldSkyboxRot = globalCtx->skyboxCtx.rot.y;
        ...
        if (globalCtx->skyboxId <= 1) {
            globalCtx->skyboxCtx.rot.y = oldSkyboxRot;
        }
    }
}
    ...
    if ((HREG(80) != 10) || (HREG(82) != 0)) {
        ...
        if (gTrnsnUnkState == 3) {
            ...
            if (R_PAUSE_MENU_MODE == 2) {
                // Set the cursor to be on the current floor for the dungeon map when entering pause
                gDungeonMapCursor = gMinimapState.dungeonFloor;

                MsgEvent_SendNullTask();

                if (DPC_PIPEBUSY_REG != 0) {
                    // only do this on real hardware for emu speedup
...
void Gameplay_InitSkybox(GlobalContext* globalCtx, s16 skyboxId) {
    func_800B0E50(&globalCtx->state, &globalCtx->skyboxCtx, skyboxId);
...
void Gameplay_InitScene(GlobalContext* globalCtx, s32 spawn) {
    ...
    // Spaceworld - reset audio for scenes that don't have any sound settings header
    if (*(u8*)globalCtx->sceneSegment != 0x15) {
        globalCtx->soundCtx.seqIndex = 0x00;
        globalCtx->soundCtx.nightSeqIndex = 0x00;
        if (gSaveContext.seqIndex == 0xFF) {
            Audio_SetBGM(0x00 | 0xF0000000);
        }
    }
...
s16 gDungeonMapCursor = MAP_FLOOR_BTN_1;

void Gameplay_SpawnScene(GlobalContext* globalCtx, s32 sceneNum, s32 spawn) {
    Scene* scene = &gSceneTable[sceneNum];

    gMinimapState.minimapToggled = false;
    gPauseDungeonMapState.mapInfoId = -1; // Force reload
...
s32 func_800C0CB8(GlobalContext* globalCtx) {
    return (globalCtx->roomCtx.curRoom.mesh->polygon.type != 1) && (YREG(15) != 0x20) && (YREG(15) != 0x30) &&
           ...
           (globalCtx->sceneNum != SCENE_PR_MARKET_1) && (globalCtx->sceneNum != SCENE_PR_OUTSIDE_TOT);
}
