...
// Uncomment the line below to quickly enable on-screen prints of current camera settings
// #define PRINT_CAM_SETTINGS

#include "z_camera_data.c"

/*===============================================================*/

// Added for debugging cameras

char* Camera_SetTextValue(s16 value, char* str, u8 endIdx) {
    char* strIter;
    char sign;
...
s32 Camera_Battle1(Camera* camera) {
    ...
    // camera doesnt pan up when targeting and charging sword
...
// Responsible for "Item2" camera setting
...
/**
 * Changed for SW since no more chest cutscene, this helps preserve Item2 cam for Ocarina
 */
s32 Camera_Demo3(Camera* camera) {
    ...
    playerHeight = LINK_IS_CHILD ? Player_GetHeight(camera->player) - 4 : Player_GetHeight(camera->player) - 12;
    camera->unk_14C &= ~0x10;
    if (RELOAD_PARAMS) {
        CameraModeValue* values = sCameraSettings[camera->setting].cameraModes[camera->mode].values;
        f32 yNormal = (1.0f + t) - ((68.0f / playerHeight) * t);
    ...
    *eye = *eyeNext;
    Camera_BGCheck(camera, at, eye);
    camera->fov = Camera_LERPCeilF(45.0f, camera->fov, camera->fovUpdateRate, 1.0f);
    camera->roll = Camera_LERPCeilS(0, camera->roll, 0.5f, 0xA);
}
        ...
        case 1:
            ...
            if (spec9->doorParams.timer1 <= 0) {
                ...
                if (params->interfaceFlags & 1) {
                    ...
                    s16 yaw;
                    // 0xE38 ~ 20 degrees
                    eyeAdjustment.pitch = 0xE38 + DEGF_TO_BINANG(-5);
                    // 0xAAA ~ 15 degrees.
                    yaw = DEGF_TO_BINANG(5) * ((camera->globalCtx->state.frames & 1) ? 1 : -1);
                    eyeAdjustment.yaw = anim->targetYaw + yaw;
                    eyeAdjustment.r = 225.0f * yNormal;
                    Camera_Vec3fVecSphGeoAdd(eyeNext, at, &eyeAdjustment);
                    spAC = *eye = *eyeNext;
...
void Camera_PrintSettings(Camera* camera) {
    ...
    if ((OREG(0) & 1) && (camera->globalCtx->activeCamera == camera->thisIdx) && !gDbgCamEnabled) {
        ...
        // Added for SW to help debug camera

        func_8006376C(4, 5, 1, "AT:");
        Camera_SetTextValue(camera->at.x, &xyzText, 7);
        func_8006376C(1, 6, 4, &xyzText);
        Camera_SetTextValue(camera->at.y, &xyzText, 7);
        func_8006376C(1, 7, 4, &xyzText);
        Camera_SetTextValue(camera->at.z, &xyzText, 7);
        func_8006376C(1, 8, 4, &xyzText);

        func_8006376C(4, 10, 1, "EYE:");
        Camera_SetTextValue(camera->eye.x, &xyzText, 7);
        func_8006376C(1, 11, 4, &xyzText);
        Camera_SetTextValue(camera->eye.y, &xyzText, 7);
        func_8006376C(1, 12, 4, &xyzText);
        Camera_SetTextValue(camera->eye.z, &xyzText, 7);
        func_8006376C(1, 13, 4, &xyzText);

        xDiff = player->actor.world.pos.x - camera->eye.x;
        zDiff = player->actor.world.pos.z - camera->eye.z;

        func_8006376C(4, 15, 1, "DIST:");
        Camera_SetTextValue((s32)sqrtf(SQ(xDiff) + SQ(zDiff)), &xyzText, 7);
        func_8006376C(1, 16, 4, &xyzText);

        // Added to debug cameras
        func_8006376C(4, 18, 1, "PITCH:");
        Camera_SetTextValue(BINANG_TO_DEGF(camera->camDir.x), &xyzText, 7);
        func_8006376C(1, 19, 4, &xyzText);
    }
#endif
}
...
Vec3s Camera_Update(Camera* camera) {
    ...
    if (camera->player != NULL) {
        ...
        if (camera->player->globalTurnFlag != 0) {
            curPlayerPosRot.rot.y = camera->player->currentYaw;
        }
    ...
    if (sOOBTimer < 200) {
        // osSyncPrintf("CAMERA FUNCTION: %8X\n",
        // sCameraFunctions[sCameraSettings[camera->setting].cameraModes[camera->mode].funcIdx]);
...
s32 Camera_ChangeModeFlags(Camera* camera, s16 mode, u8 flags) {
    ...
    if (!((sCameraSettings[camera->setting].unk_00 & 0x3FFFFFFF) & (1 << mode))) {
        ...
        // Was once changed for SW, is now removed for SW because these sounds are now played by ShrinkWindow_Update
        /*         if (camera->status == CAM_STAT_ACTIVE) {
                    switch (modeChangeFlags) {
                        case 1:
                            // func_80078884(NA_SE_SY_DUMMY_13); // Spaceworld Addition for Letterbox Disappear,
           func_80078884(0); break; case 2: if (camera->globalCtx->roomCtx.curRoom.unk_03 == 1) {
                                // Audio_PlaySoundPitched(NA_SE_SY_HP_RECOVER); // Spaceworld Addition for Letterbox
           Appear,
                                                                             //
           func_80078884(NA_SE_SY_ATTENTION_URGENCY); } else {
                                // Audio_PlaySoundPitched(NA_SE_SY_HP_RECOVER); // Spaceworld Addition for Letterbox
           Appear,
                            ...
                            break;
                        case 4:
                            // Audio_PlaySoundPitched(NA_SE_SY_HP_RECOVER); // Spaceworld Addition for Letterbox Appear,
