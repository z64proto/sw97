...
void func_80098904(GlobalContext* globalCtx, SceneCmd* cmd) {
    ...
    // rearranged structure from the final game, apparently?
    // most of these flags don't actually exist yet.

    globalCtx->roomCtx.curRoom.unk_03 = cmd->roomBehavior.gpFlag2;
    globalCtx->roomCtx.curRoom.unk_02 = 0;
    globalCtx->roomCtx.curRoom.showInvisActors = 0;
    globalCtx->msgCtx.unk_E40C = 0;
}
...
void func_80098D5C(GlobalContext* globalCtx, SceneCmd* cmd) {
    ...
    // Spaceworld
    if (globalCtx->skyboxId == 0x00 && !globalCtx->envCtx.skyDisabled) {
        globalCtx->skyboxId = 0x1D;
    }
}
...
void func_80098D80(GlobalContext* globalCtx, SceneCmd* cmd) {
    ...
    // Spaceworld change to make nighttime sfx play
    if ((cmd->timeSettings.hour == 0x00) && (cmd->timeSettings.min == 0x00)) {
        cmd->timeSettings.hour = 0xFF;
        cmd->timeSettings.min = 0xFF;
    }
    ...
    if (gSaveContext.unk_1422 == 0) {
        gTimeSpeed = globalCtx->envCtx.unk_02;
    }
...
void func_80099140(GlobalContext* globalCtx, SceneCmd* cmd) {
    ...
    // Spaceworld change to enforce daytime bgm for dungeons that want to play nighttime sfx
    if (globalCtx->soundCtx.nightSeqIndex == 0x00 && gSaveContext.gameMode == 0) {
        if (globalCtx->sceneNum == SCENE_DODONGOS_CAVERN || globalCtx->sceneNum == SCENE_DEKU_TREE ||
            globalCtx->sceneNum == SCENE_JABU_TEST) {
            globalCtx->soundCtx.nightSeqIndex = 0x13; // will always play daytime bgm irregardless of the time
        }
    }

    // Change nightime/nature sfx to some more fitting ones
    switch (globalCtx->soundCtx.nightSeqIndex) {
        case 0x01: // Kakariko Village
            globalCtx->soundCtx.seqIndex = 0x7F;
            globalCtx->soundCtx.nightSeqIndex = 0x08;
            break;
        case 0x03: // Lost Woods
            globalCtx->soundCtx.seqIndex = 0x7F;
            globalCtx->soundCtx.nightSeqIndex = 0x04;
            break;
        case 0x04: // Kokiri Forest
            globalCtx->soundCtx.seqIndex = 0x7F;
            globalCtx->soundCtx.nightSeqIndex = 0x0F; // 0x05 crickets
            break;
        case 0x05: // Sacred Forest Meadow, ToT Exterior, Market Entrance
            globalCtx->soundCtx.seqIndex = 0x7F;
            globalCtx->soundCtx.nightSeqIndex = 0x02;
            break;
        case 0x06: // Lake Hylia
            globalCtx->soundCtx.seqIndex = 0x7F;
            globalCtx->soundCtx.nightSeqIndex = 0x08;
            break;
        case 0x07: // Zora's River
            globalCtx->soundCtx.seqIndex = 0x7F;
            globalCtx->soundCtx.nightSeqIndex = 0x08;
            break;
        case 0x09: // Gerudo Valley
            globalCtx->soundCtx.seqIndex = 0x7F;
            globalCtx->soundCtx.nightSeqIndex = 0x01;
            break;
    }
...
RomFile gObjectTable[] = {
    ...
    ROM_FILE(object_syokudai), // ROM_FILE(object_torch2),
    ROM_FILE(object_bl),
    ROM_FILE(object_tp),
    ROM_FILE(object_oA1),
    ROM_FILE(object_st),
