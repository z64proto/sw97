...
static Gfx* D_80115484[] = {
    ...
    gUnusedTreeStumpDL,
    gSignRectDL, // gSignRectangularDL
    gSignDl,     // gSignDirectionalDl
    gBoulderFragmentsDL,
};
...
void EnAObj_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    switch (thisx->params) {
        ...
        case A_OBJ_UNKNOWN_6:
            // this actor has invalid collision and dlist
            // so we'll replace it with the hookshot target it used to be ;)
            Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_OBJ_HSBLOCK, thisx->world.pos.x, thisx->world.pos.y,
    ...
    if (this->dyna.actor.textId != 0) {
        ...
        if (ABS(var) < 0x2800) {
            if (Actor_IsTalking(&this->dyna.actor, globalCtx)) {
                ...
                Actor_RequestToTalk(&this->dyna.actor, globalCtx);
            }
        }
    }
}
...
void EnAObj_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->params == A_OBJ_KNOB) {
        gDPSetPrimColor(POLY_OPA_DISP++, 0, 1, 255, 255, 255, 50);
    }
