...
#include "physics_data.h" // data for hat physics
...
u8 gGettingOffLadder = false;
u8 gKeepItemsOut = false; // hacky way to make sure the change only happens for getting on epona
...
static u8 sActionModelGroups[] = {
    3, 15, 10, 2, 2, 5,  10, 11, 6,  6,  6,  6,  6,  6,  6,  6,  9,  9,  7, 7,  8, 3,  3,
    3, 3,  3,  3, 3, 12, 13, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 3, 14, 3, 3,  3,
    3, 3,  3,  3, 3, 3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3, 3,  3, 16, 3,
};
...
u8 gPlayerModelTypes[][5] = {
    { 2, 0, 10, 16, 20 }, { 1, 2, 9, 19, 20 },  { 1, 2, 10, 17, 20 }, { 0, 0, 8, 18, 20 },  { 0, 0, 8, 18, 20 },
    { 3, 4, 9, 19, 20 },  { 4, 1, 11, 18, 20 }, { 5, 0, 8, 18, 20 },  { 0, 6, 8, 18, 20 },  { 4, 0, 15, 18, 20 },
    { 3, 1, 9, 18, 20 },  { 3, 5, 9, 18, 20 },  { 0, 0, 13, 18, 20 }, { 0, 0, 14, 18, 20 }, { 0, 7, 8, 18, 20 },
    { 0, 2, 8, 19, 20 },  { 2, 0, 10, 16, 20 },
};
...
static Gfx* sPlayerWaistDLists[] = {
    NULL, // gLinkAdultWaistNearDL,
    NULL, // gLinkChildWaistNearDL,
    NULL, // gLinkAdultWaistNearDL,
    NULL, // gLinkChildWaistNearDL,
};
...
static Gfx* sPlayerShoulderDLists[] = {
    gLinkAdultSkel_bone013_gLinkAdultLeftShoulderLimb_mesh_layer_Opaque,
    gLinkChildSkel_bone013_gLinkChildRightSholderLimb_mesh_layer_Opaque,
};
...
u8 gShieldOn = false;
u8 gIsCrouching = false;
u8 gIsFullHeartPiece = false;
...
void Player_SetModelsForHoldingShield(Player* this) {
    if ((this->stateFlags1 & 0x400000) &&
        ...
        if (!Player_HoldsTwoHandedWeapon(this) && !Player_IsChildWithHylianShield(this)) {
            ...
            // Space World change: Make shield stay out after using it
            if (this->heldItemActionParam == PLAYER_AP_SWORD_KOKIRI ||
                this->heldItemActionParam == PLAYER_AP_SWORD_MASTER ||
                this->heldItemActionParam == PLAYER_AP_SWORD_BGS || gIsCrouching ||
...
#else
    // Return max strength for Space World
...
s32 Player_ActionToMagicSpell(Player* this, s32 actionParam) {
    s32 magicSpell = actionParam - PLAYER_AP_MAGIC_SPELL_ICE;
...
void Player_HatPhysicsCallback(s32 limbIndex, GlobalContext* globalCtx, void* argNull) {
    if (limbIndex != 0) {
        return;
    }
    ...
    Matrix_RotateX_f(90.0f, MTXMODE_APPLY);
    Matrix_RotateY_f(sHatPhysicsStrand[gSaveContext.linkAge].rigidity.rot.y, MTXMODE_APPLY);
    Matrix_RotateZ_f(sHatPhysicsStrand[gSaveContext.linkAge].rigidity.rot.z, MTXMODE_APPLY);
}
...
static void* sEyeTextures[][8] = {
    {
        ...
        gLinkAdultEyesLookDownTex,
        gLinkAdultEyesHurtTex,
    },
    {
        ...
        gLinkChildEyesLookDownTex,
        gLinkChildEyesHurtTex,
    },
};

static void* sMouthTextures[][4] = {
    {
    ...
    {
        gLinkChildMouthClosedTex,
        gLinkChildMouthGrittingTeethTex,
        gLinkChildMouthShoutTex,
        gLinkChildMouthSmileTex,
    },
};

// Hardcoded checks to determine when to use physics for the hat and when to use the static model
s32 Player_UsePhysics(GlobalContext* globalCtx, Player* player) {
    u32 a = (u32)player->skelAnime.animation;

    if (a == 0x040033D0 || a == 0x04002400 || a == 0x04002878 || a == 0x040657B0 || a == 0x04003400 ||
        a == 0x040033D8 || a == 0x040033F8 ||
        (a == 0x04002878 /*&& player->skelAnime.playSpeed < 0.0f && player->skelAnime.curFrame >= 45.0f*/)) {
        return false;
    }
    return true;
}
...
void Player_DrawImpl(GlobalContext* globalCtx, void** skeleton, Vec3s* jointTable, s32 dListCount, s32 lod, s32 tunic,
    ...
    // make link ignore blink data in the animation for recieving an item
    if (anim == (LinkAnimationHeader*)0x04002780 || eyeIndex < 0) {
    ...
    sDListsLodOffset = 0; // always use near dlist for spaceworld

#if 0
    SkelAnime_DrawFlexLod(globalCtx, skeleton, jointTable, dListCount, overrideLimbDraw, postLimbDraw, data, lod);
#else
    // Remove LOD limbs for Space World
    SkelAnime_DrawFlexOpa(globalCtx, skeleton, jointTable, dListCount, overrideLimbDraw, postLimbDraw, data);
#endif
    ...
    if ((overrideLimbDraw != Player_OverrideLimbDraw2) && (overrideLimbDraw != Player_OverrideLimbDraw3) &&
        ...
        if (player->heldItemActionParam == PLAYER_AP_BOMB || player->heldItemActionParam == PLAYER_AP_BOMBCHU ||
            player->heldItemActionParam == PLAYER_AP_BOMB_BAG) {
        ...
        if (LINK_IS_ADULT) {
#if 0
            // Remove gauntlets on adult for Space World
            s32 strengthUpgrade = CUR_UPG_VALUE(UPG_STRENGTH);
            ...
            if (!gSaveContext.fairyTimer && boots != 0) {
...
#if 0
        // Remove Goron Braclet for Space World
        else {
...

    {
        ...
        // Align the sphere collision to the body parts of the player
        sPhysicsSphereCenterList[0] = player->bodyPartsPos[LINK_PARTS_HEAD];
        sPhysicsSphereCenterList[1] = player->bodyPartsPos[LINK_PARTS_CHEST1];
        sPhysicsSphereCenterList[2] = player->bodyPartsPos[LINK_PARTS_L_ARM_A];
        sPhysicsSphereCenterList[3] = player->bodyPartsPos[LINK_PARTS_L_ARM_B];
        sPhysicsSphereCenterList[4] = player->bodyPartsPos[LINK_PARTS_R_ARM_A];
        sPhysicsSphereCenterList[5] = player->bodyPartsPos[LINK_PARTS_R_ARM_B];

//#define DEBUG_HAT_OFFSET
#ifdef DEBUG_HAT_OFFSET
        {
            Input* controller1 = &globalCtx->state.input[0];

            if (CHECK_BTN_ALL(controller1->cur.button, BTN_DUP)) {
                sHatOffsets[gSaveContext.linkAge].x += 0.1f;
            } else if (CHECK_BTN_ALL(controller1->cur.button, BTN_DDOWN)) {
                sHatOffsets[gSaveContext.linkAge].x -= 0.1f;
            } else if (CHECK_BTN_ALL(controller1->cur.button, BTN_DLEFT)) {
                sHatOffsets[gSaveContext.linkAge].y += 0.1f;
        ...
        sHatPhysicsStrand[gSaveContext.linkAge].info.floorY = BgCheck_EntityRaycastFloor4(
            &globalCtx->colCtx, &player->actor.floorPoly, &bgId, &player->actor, &player->actor.world.pos);

        // Draw the hat (but don't when fairy spell is happening)
        if (!gSaveContext.fairyTimer) {
            if (!Player_UsePhysics(globalCtx, PLAYER)) {
                Gfx* hatDL = LINK_IS_ADULT ? gSWLinkAdultStaticHatDL : gLinkChildStaticHatDL;
                gSPDisplayList(POLY_OPA_DISP++, hatDL);
            } else {
                POLY_OPA_DISP = Physics_DrawDynamicStrand(globalCtx->state.gfxCtx, POLY_OPA_DISP, sHatPhysicsJoints,
                                                          &sHatPhysicsStrand[gSaveContext.linkAge],
                                                          Player_HatPhysicsCallback, globalCtx, NULL);
            }
        }
    }
        ...
        if (sp98.y < posB) {
            ...
            if ((temp3 >= 2) && (temp3 < 4) && !SurfaceType_IsWallDamage(&globalCtx->colCtx, sp88, sp84)) {
                ...
                if (globalCtx->sceneNum != SCENE_FSTDAN) {
                    // Remove lava in fst_dan for Space World
    ...
    if (!Player_OverrideLimbDrawImpl(globalCtx, limbIndex, dList, pos, rot, thisx)) {
        if (limbIndex == PLAYER_LIMB_L_HAND) {
            ...
            *dList = dLists[sDListsLodOffset];
            ...
            // remove the hat since its replaced with the physics hat
            *dList = NULL;
        }
...
void Player_SetShieldCollision(GlobalContext* globalCtx, Player* this, ColliderQuad* collider, Vec3f* quadSrc) {
    ...
    if ((this->stateFlags1 & 0x400000) || gShieldOn) {
        Vec3f quadDest[4];
...
void Player_SetSwordAttack(GlobalContext* globalCtx, Player* this, Vec3f* newTipPos) {
    ...
    // for spaceworld, make spin attack work all the time without worrying about stateFlags2
...
void Player_DrawGetItemImpl(GlobalContext* globalCtx, Player* this, Vec3f* refPos, s32 drawIdPlusOne) {
    ...
    gSegments[7] = VIRTUAL_TO_PHYSICAL(this->giObjectSegment);

    // gSPSegment(POLY_OPA_DISP++, 0x08, this->giObjectSegment);
    gSPSegment(POLY_XLU_DISP++, 0x07, this->giObjectSegment);

    Matrix_Translate(refPos->x + (3.3f * Math_SinS(this->actor.shape.rot.y)), refPos->y + 11.0f,
...
void Player_DrawHookshotPointer(GlobalContext* globalCtx, Player* this, f32 arg2) {
    ...
    if (BgCheck_AnyLineTest3(&globalCtx->colCtx, &posA, &posB, &aimProjectedPos, &polygon, 1, 1, 1, 1, &bgId)) {
        ...
        SkinMatrix_Vec3fMtxFMultXYZW(&globalCtx->projectionMatrix, &aimProjectedPos, &screenPos, &w);
...
void Player_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        ...
        if (this->itemActionParam == PLAYER_AP_STICK) {
            ...
            // for spaceworld, shorten the kokiri sword. index 2, original value: 3000
            static f32 swordLengths[] = {
                0.0f, 4000.0f, 1800.0f, 5500.0f, 0.0f, 2500.0f,
            };
            Vec3f swordTip[3];
        ...
        if (this->actor.scale.y >= 0.0f) {
            ...
            if ((this->unk_862 != 0) || ((func_8002DD6C(this) == 0) && (heldActor != NULL))) {
                if (!(this->stateFlags1 & 0x400) && (this->unk_862 != 0) && (this->exchangeItemId != EXCH_ITEM_NONE)) {
                    ...
                    sGetItemRefPos.x = (this->bodyPartsPos[LINK_PARTS_R_HAND].x + this->leftHandPos.x) * 0.5f;
                    sGetItemRefPos.y = (this->bodyPartsPos[LINK_PARTS_R_HAND].y + this->leftHandPos.y) * 0.5f;
                    sGetItemRefPos.z = (this->bodyPartsPos[LINK_PARTS_R_HAND].z + this->leftHandPos.z) * 0.5f;
                }
        ...
        if (limbIndex == PLAYER_LIMB_SHEATH) {
            ...
            // Physics Hat
            Physics_GetHeadProperties(&sHatPhysicsStrand[gSaveContext.linkAge], &sHatOffsets[gSaveContext.linkAge],
                                      false);
        } else {
...
void Player_PauseDrawImpl(GlobalContext* globalCtx, void* seg04, void* seg06, SkelAnime* skelAnime, Vec3f* pos,
    ...
    Gfx_DrawHiliteReflectionOpa(pos, &globalCtx->view.eye, &lightDir, globalCtx->state.gfxCtx);
