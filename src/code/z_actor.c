...
NaviColor sNaviColorList[] = {
    ...
    { { 255, 255, 0, 255 }, { 255, 255, 255, 0 } }, { { 255, 0, 0, 255 }, { 200, 0, 0, 0 } }, // red for enemies
    { { 150, 150, 255, 255 }, { 0, 255, 0, 0 } },   { { 0, 255, 0, 255 }, { 0, 255, 0, 0 } },
    { { 0, 255, 0, 255 }, { 0, 255, 0, 0 } },       { { 255, 0, 0, 255 }, { 200, 0, 0, 0 } }, // red for bosses
    { { 0, 255, 0, 255 }, { 0, 255, 0, 0 } },       { { 0, 255, 0, 255 }, { 0, 255, 0, 0 } },
...
#define TARGET_VTX_MAX (16) // tweak thickness here
#define TARGET_VTX_MIN (-(32) - 1)
    static Vtx sBetaTargetVertices[6] = {
        VTX(0, 0, 0, 512, 512, 255, 255, 255, 255),
        VTX(0, TARGET_VTX_MIN, 0, 512, 1024, 255, 255, 255, 255),
        VTX(TARGET_VTX_MAX, TARGET_VTX_MIN, 0, 0, 1024, 255, 255, 255, 255),
        VTX(TARGET_VTX_MAX, TARGET_VTX_MAX, 0, 0, 0, 255, 255, 255, 255),
        VTX(TARGET_VTX_MIN, TARGET_VTX_MAX, 0, 1024, 0, 255, 255, 255, 255),
        VTX(TARGET_VTX_MIN, 0, 0, 1024, 512, 255, 255, 255, 255),
    };
    static Gfx sBetaTargetDL[] = {
        gsSPVertex(sBetaTargetVertices, ARRAY_COUNT(sBetaTargetVertices), 0),
        gsSP2Triangles(0, 1, 3, 0, 1, 2, 3, 0),
        gsSP2Triangles(0, 3, 5, 0, 3, 4, 5, 0),
        gsSPEndDisplayList(),
    };
#undef TARGET_VTX_MAX
#undef TARGET_VTX_MIN
    ...
    if (targetCtx->unk_48 != 0) {
        ...
        // Changed for SW, don't hide reticle if player is talking
            ...
            if (entry->unk_0C < 500.0f) {
                ...
                for (i = 0; i < 4; i++) {
                    ...
                    gSPMatrix(OVERLAY_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_actor.c", 2116),
                              G_MTX_MODELVIEW | G_MTX_LOAD);
                    gSPDisplayList(OVERLAY_DISP++, sBetaTargetDL); // replaced with custom dl
                    Matrix_Pop();
                }
            }
...
void func_8002C7BC(TargetContext* targetCtx, Player* player, Actor* actorArg, GlobalContext* globalCtx) {
    ...
    // Changed for SW; if targeting an actor, Navi will hover over the next targetable actor in the area instead (as
    // seen in footage)
    else if ((actorArg != NULL) && (unkActor == NULL)) {
        unkActor = actorArg;
    }
    ...
    // Added for SW; if Link is currently talking and Navi isn't hovering over another nearby actor...
    if ((player->stateFlags1 & (1 << 6)) && (unkActor == actorArg)) {
        // Make sure that the talking actor is nearby and isn't somehow nonexistent
        nearbyActor = player->targetActor;
        if (nearbyActor != NULL) {
            // Force the current target actor to be the talking actor
            unkActor = nearbyActor;
            actorCategory = nearbyActor->category;
        }
    }

    // Added for SW; If Link is currently getting an item...
    if (player->stateFlags1 & (1 << 10)) {
        // Check if he's near a chest
        nearbyActor = Actor_FindNearby(globalCtx, &player->actor, ACTOR_EN_BOX, ACTORCAT_CHEST, 140.0f);
        if (nearbyActor != NULL) {
            // If he's near a chest, force current target actor to be the chest
            unkActor = nearbyActor;
            actorCategory = nearbyActor->category;
        }
    }
    ...
    // If Navi's target actor is NULL, make her target the player instead
    if (unkActor == NULL) {
        unkActor = &player->actor;
    }

    // Move Navi to the target actor
    ...
    if (actorArg != NULL) {
        ...
        // Changed for SW, make sure letterbox has fully appeared on screen first
        if (ShrinkWindow_GetCurrentVal() >= 0x1B) {
            ...
            if (targetCtx->unk_4B == 0) {
                ...
                if (Math_StepToF(&targetCtx->unk_44, 80.0f, temp6) != 0) {
                    targetCtx->unk_4B++;
                    // Changed timing for reticle sound for SW
                    if ((actorArg->flags & 5) == 5) {
                        func_80078884(NA_SE_SY_DUMMY_14);
                    } else {
                        Audio_PlaySoundPitched(NA_SE_SY_LOCK_ON_HUMAN);
                    }
                }
            } else {
...
void Actor_MoveForwardXYZ(Actor* actor) {
    Actor_SetMovementSpeedXYZ(actor);
    Actor_Move(actor);
}
...
s32 Actor_IsFacingPlayer(Actor* actor, s16 arg1) {
...
s32 Actor_ActorAIsFacingActorB(Actor* actorA, Actor* actorB, s16 arg2) {
...
s32 Actor_IsFacingAndNearPlayer(Actor* actor, f32 arg1, s16 arg2) {
    s16 var = actor->yawTowardsPlayer - actor->shape.rot.y;
...
s32 Actor_ActorAIsFacingAndNearActorB(Actor* actorA, Actor* actorB, f32 arg2, s16 arg3) {
...
// & 0x04 - check floor
// & 0x08 - snap to floor
// & 0x40 - disable water ripples
// & 0x80 - use wall check 3 (skips some checks if moving in the xz plane?)
...
TargetRangeParams D_80115FF8[] = {
    /* 0 */ TARGET_RANGE(70, 140),
    /* 1 */ TARGET_RANGE(170, 255),
    /* 2 */ TARGET_RANGE(280, 5600),
    /* 3 */ TARGET_RANGE(350, 525),
    /* 4 */ TARGET_RANGE(700, 1050),
    /* 5 */ TARGET_RANGE(1000, 1500),
    /* 6 */ TARGET_RANGE(100, 105.36842),
    /* 7 */ TARGET_RANGE(140, 163.33333),
    /* 8 */ TARGET_RANGE(240, 576),
    /* 9 */ TARGET_RANGE(280, 280000),
};
...
s32 Actor_RequestToTalkAndExchangeItemInRange(Actor* actor, GlobalContext* globalCtx, f32 arg2, f32 arg3,
                                              u32 exchangeItemId) {
    Player* player = PLAYER;
...
s32 Actor_RequestToTalkAndExchangeItem(Actor* actor, GlobalContext* globalCtx, f32 arg2, u32 exchangeItemId) {
    return Actor_RequestToTalkAndExchangeItemInRange(actor, globalCtx, arg2, arg2, exchangeItemId);
}

s32 Actor_RequestToTalkInRange(Actor* actor, GlobalContext* globalCtx, f32 arg2) {
    return Actor_RequestToTalkAndExchangeItem(actor, globalCtx, arg2, EXCH_ITEM_NONE);
}

s32 Actor_RequestToTalk(Actor* actor, GlobalContext* globalCtx) {
    f32 var1 = 50.0f + actor->colChkInfo.cylRadius;

    return Actor_RequestToTalkInRange(actor, globalCtx, var1);
}

u32 Actor_HasFinishedTalking(Actor* actor, GlobalContext* globalCtx) {
...
void Actor_GetProjectionPos(GlobalContext* globalCtx, Actor* actor, s16* arg2, s16* arg3) {
    Vec3f sp1C;
    f32 sp18;

    func_8002BE04(globalCtx, &actor->focus.pos, &sp1C, &sp18);
    *arg2 = sp1C.x * sp18 * (SCREEN_WIDTH / 2) + (SCREEN_WIDTH / 2);
    *arg3 = sp1C.y * sp18 * -(SCREEN_HEIGHT / 2) + (SCREEN_HEIGHT / 2);
}
        ...
        if ((((player->heldActor != NULL) || (actor == player->targetActor)) && (getItemId > GI_NONE) &&
            ...
            if ((actor->xzDistToPlayer < xzRange) && (fabsf(actor->yDistToPlayer) < yRange)) {
                ...
                if ((getItemId != GI_NONE) || (player->getItemDirection < absYawDiff)) {
                    if (getItemId == GI_HEART_CONTAINER_2) {
                        Item_Give(globalCtx, ITEM_HEART_CONTAINER);
                        return true;
                    }
...
// Flags for each actor category used to determine when to stop running the update function
u32 D_80116068[] = {
            ...
            if (actor->init != NULL) {
                ...
                actor = actor->next;
                        ...
                        (!unkFlag && unkCondition && (sp74 != actor) && (actor != player->naviActor) &&
                         ...
                         !((actor->params != 0x03) && (actor->id == ACTOR_EN_ELF))))) {
                ...
                // manually overwrite xz and y dist to link so enemies think link is far away when using dark magic
                if ((actor->category == ACTORCAT_ENEMY) && (gSaveContext.nayrusLoveTimer != 0) &&
                    !((actor->id == ACTOR_EN_MB) && (actor->params == 0x0000))) {
                    actor->xzDistToPlayer = 1000.0f * Actor_WorldDistXZToActor(actor, &player->actor);
                    actor->yDistToPlayer = 1000.0f * Actor_HeightDiff(actor, &player->actor);
                    actor->xyzDistToPlayerSq = SQ(actor->xzDistToPlayer) + SQ(actor->yDistToPlayer);

                    actor->yawTowardsPlayer = actor->yawTowardsPlayer + 1800;
                } else {
...
void func_80030FA8(GraphicsContext* gfxCtx) {
    ...
    // Spaceworld change: smaller lens of truth
    gSPTextureRectangle(POLY_XLU_DISP++, 0 << 2, 0 << 2, 320 << 2, 240 << 2, G_TX_RENDERTILE, (0 << 5), (0 << 5),
                        1 << 10, 1 << 10);
...
Actor* Actor_Spawn(ActorContext* actorCtx, GlobalContext* globalCtx, s16 actorId, f32 posX, f32 posY, f32 posZ,
    ...
    // Spawn a green cube if the actor is invalid
    if (actorId != ACTOR_PLAYER) {
        if (gActorOverlayTable[actorId].initInfo == NULL) {
            actorId = ACTOR_GREEN_CUBE;
            params = 0;
        }
    }
    ...
    // hacky behaviour for spaceworld old skeletons
    if (actorId == ACTOR_EN_TEST && objBankIndex < 0) {
        objBankIndex = Object_GetIndex(&globalCtx->objectCtx, OBJECT_SKELETON);
        params = 0x0003;
    }

    if (objBankIndex < 0) {
        Actor_FreeOverlay(overlayEntry);
        // Spawn a red cube if the object doesn't exist (currently disabled)
        return NULL; // Actor_Spawn(actorCtx, globalCtx, ACTOR_GREEN_CUBE, posX, posY, posZ, rotX, rotY, rotZ, 0x0000);
    }
    ...
    ASSERT(overlayEntry->nbLoaded < 255);
...
Actor* Actor_Delete(ActorContext* actorCtx, Actor* actor, GlobalContext* globalCtx) {
    ...
    if (overlayEntry->vramStart == 0) {
        ...
        ASSERT(overlayEntry->loadedRamAddr != NULL);

        ASSERT(overlayEntry->nbLoaded > 0);
...
// Changed for SW, tweaks what is considered "on screen" to be targetable
s32 func_80032880(GlobalContext* globalCtx, Actor* actor) {
    s16 x;
    s16 y;

    Actor_GetProjectionPos(globalCtx, actor, &x, &y);

    return (-214 < x && x < (SCREEN_WIDTH + 214) && -160 < y && y < (SCREEN_HEIGHT + 160));
}
...
Actor* func_80032AF0(GlobalContext* globalCtx, ActorContext* actorCtx, Actor** actorPtr, Player* player) {
    ...
    // allow navi to hover over things even if player is busy
...
void Actor_SpawnFlyingDust(GlobalContext* globalCtx, Vec3f* arg1, f32 arg2, s32 arg3, s16 arg4, s16 scaleStep,
                           u8 arg6) {
    Vec3f pos;
    Vec3f velocity = { 0.0f, 0.0f, 0.0f };
    Vec3f accel = { 0.0f, 0.3f, 0.0f };
...
Actor* Actor_FindNearbyExplosion(GlobalContext* globalCtx, Actor* explosiveActor) {
...
Actor* Actor_GetProjectileActor(GlobalContext* globalCtx, Actor* refActor, f32 arg2) {
...
void Gfx_DrawCircleShadow(Vec3f* arg0, Vec3f* arg1, u8 alpha, GlobalContext* globalCtx) {
    MtxF sp60;
    f32 var;
    Vec3f sp50;
    CollisionPoly* sp4C;
...
s32 Actor_Talk(GlobalContext* globalCtx, Actor* actor, s16* arg2, f32 arg3, callback1_800343CC unkFunc1,
...
typedef struct {
    /* 0x00 */ NpcTurnInfo sub_00;
    /* 0x10 */ f32 unk_10;
    /* 0x14 */ s16 unk_14;
} NpcSightInfo; // size = 0x18

NpcSightInfo sNpcSightInfo[] = {
...
s16 Npc_GetSightAngleRange(s16 arg0) {
    return sNpcSightInfo[arg0].unk_14;
}
...
void Npc_TurnTowardsFocus(Actor* actor, NpcInfo* npcInfo, s16 arg2, s16 arg3) {
    NpcTurnInfo sp38;

    npcInfo->turnState = func_80034810(actor, npcInfo, sNpcSightInfo[arg2].unk_10, sNpcSightInfo[arg2].unk_14, arg3);

    sp38 = sNpcSightInfo[arg2].sub_00;
    ...
    Npc_TurnTowardsFocusImpl(actor, npcInfo, sp38.unk_00, sp38.unk_04, sp38.unk_02, sp38.unk_06, sp38.unk_0A,
                             sp38.unk_08, sp38.unk_0C);
}
...
s16 Npc_GetFadeOutAlpha(Actor* actor, GlobalContext* globalCtx, s16 arg2, f32 arg3) {
    Player* player = PLAYER;
    f32 var;
