...
void func_80095D04(GlobalContext* globalCtx, Room* room, u32 flags) {
    ...
    for (sp9C = 0; sp9C < polygon2->num; sp9C++, polygonDlist++) {
        ...
        SkinMatrix_Vec3fMtxFMultXYZW(&globalCtx->projectionMatrix, &sp90, &sp84, &sp80);
...
void func_8009638C(Gfx** displayList, u32 source, u32 tlut, u16 width, u16 height, u8 fmt, u8 siz, u16 mode0,
    ...
    func_80096238(SEGMENTED_TO_VIRTUAL(source)); // jpeg decoding?
...
s32 func_8009728C(GlobalContext* globalCtx, RoomContext* roomCtx, s32 roomNum) {
    ...
    if (roomCtx->status == 0 && !(roomNum > globalCtx->nbRooms)) {
        ...
        ASSERT(roomNum < globalCtx->nbRooms);
...
void Room_Draw(GlobalContext* globalCtx, Room* room, u32 flags) {
    if (room->segment != NULL) {
        ...
        ASSERT(room->mesh->polygon.type < ARRAY_COUNT(sRoomDrawHandlers));
...
void func_80097534(GlobalContext* globalCtx, RoomContext* roomCtx) {
    ...
    Actor_SpawnTransitionActors(globalCtx, &globalCtx->actorCtx);
    if (gMinimapState.mapInfoId >= 0) {
        Map_SetPaletteData(globalCtx, &gMinimapState, &gMapInfoTable[gMinimapState.mapInfoId]);
    }
