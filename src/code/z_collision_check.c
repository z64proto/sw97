...
void CollisionCheck_HitSolid(GlobalContext* globalCtx, ColliderInfo* info, Collider* collider, Vec3f* hitPos) {
    ...
    if (flags == TOUCH_SFX_NORMAL && collider->colType != COLTYPE_METAL) {
        ...
        // no metal hit mark in spaceworld
...
static ColChkBloodFunc sBloodFuncs[] = {
    ...
    CollisionCheck_WaterBurst, CollisionCheck_RedBlood,
};
...
void CollisionCheck_HitEffects(GlobalContext* globalCtx, Collider* at, ColliderInfo* atInfo, Collider* ac,
    ...
    // in spaceworld links shield always spawns sparks
    if (ac == &player->shieldQuad.base) {
        if (ac->actor == NULL) {
            CollisionCheck_SpawnShieldParticlesMetal(globalCtx, hitPos);
        } else {
            CollisionCheck_SpawnShieldParticlesMetalSound(globalCtx, hitPos, &ac->actor->projectedPos);
        }
        return;
    }

    // in spaceworld links slingshot spawns sparks when hitting enemies with solid ac collision type
    if ((player->heldItemActionParam == PLAYER_AP_SLINGSHOT) && (sHitInfo[ac->colType].effect == HIT_SOLID)) {
        CollisionCheck_SpawnShieldParticlesMetal(globalCtx, hitPos);
        return;
    }

    // in spaceworld links stick always spawns dust
    if ((player->heldItemActionParam == PLAYER_AP_STICK) &&
        ((at == &player->swordQuads[0].base) || (at == &player->swordQuads[1].base))) {
        if (at->actor == NULL) {
            Audio_PlaySoundGeneral(NA_SE_IT_REFLECTION_WOOD, &D_801333D4, 4, &D_801333E0, &D_801333E0, &D_801333E8);
        } else {
            Audio_PlaySoundGeneral(NA_SE_IT_REFLECTION_WOOD, &at->actor->projectedPos, 4, &D_801333E0, &D_801333E0,
                                   &D_801333E8);
        }
        EffectSsHitMark_SpawnFixedScale(globalCtx, EFFECT_HITMARK_DUST, hitPos);
        return;
...
void CollisionCheck_ApplyDamage(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx, Collider* collider,
    ...
    // remove the check for AC_HARD in spaceworld for dealing damage
    // this change was made primarily for dodongos. if it breaks anything we'll need to revisit
...
void CollisionCheck_SpawnShieldParticlesMetalSound(GlobalContext* globalCtx, Vec3f* v, Vec3f* pos) {
    CollisionCheck_SpawnShieldParticles(globalCtx, v);
    // spaceworld change to switch between regular recoil sound and lower pitched wall recoil sfx
