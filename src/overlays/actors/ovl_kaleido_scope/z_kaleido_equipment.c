...
static s16 sPlayerRenderTexHeight[] = { 120, 96 };

static void* sPlayerRenderTex[] = { gAdultLinkPrerenderTex, gChildLinkPrerenderTex };

static void* sEquipmentBackdrops[] = {
    gSwordLeftTex,   gSwordRightTex,   gShieldLeftTex, gShieldRightTex,
    gClothesLeftTex, gClothesRightTex, gBootsLeftTex,  gBootsRightTex,
};
...
void KaleidoScope_DrawEquipment(GlobalContext* globalCtx) {
    ...
    // background text for equip
    for (j = 120, i = 0; i < ARRAY_COUNT(sEquipmentBackdrops); j += 4, i++) {
        gSPVertex(POLY_OPA_DISP++, &pauseCtx->equipVtx[j], 4, 0);
        gDPLoadTextureBlock(POLY_OPA_DISP++, sEquipmentBackdrops[i], G_IM_FMT_RGBA, G_IM_SIZ_16b, 48, 32, 0,
    ...
    if ((pauseCtx->state == 6) && (pauseCtx->unk_1E4 == 0) && (pauseCtx->pageIndex == PAUSE_EQUIP)) {
                    ...
                    if (pauseCtx->cursorX[PAUSE_EQUIP] != 0) {
                        ...
                        if (pauseCtx->cursorX[PAUSE_EQUIP] == 0) {
                            if (pauseCtx->cursorY[PAUSE_EQUIP] == 0) {
                                if ((CUR_UPG_VALUE(UPG_BULLET_BAG) != 0) && LINK_IS_ADULT) {
                                    cursorMoveResult = 1;
                                }
                            } else {
                            ...
                            // Spaceworld - Allow the cursor to go to empty slots
            ...
            while (cursorMoveResult == 0) {
                if (pauseCtx->stickRelY > 30) {
                    if (pauseCtx->cursorY[PAUSE_EQUIP] != 0) {
                        ...
                        if (pauseCtx->cursorX[PAUSE_EQUIP] == 0) {
                            if (pauseCtx->cursorY[PAUSE_EQUIP] == 0) {
                                // Spaceworld - Disable bullet bag as child
                                if ((CUR_UPG_VALUE(UPG_BULLET_BAG) != 0) && LINK_IS_ADULT) {
                                    cursorMoveResult = 1;
                                }
            ...
            if (pauseCtx->stickRelX < -30) {
                ...
                while (true) {
                    if (cursorX == 0) {
                        ...
                        // Spaceworld - Allow the cursor to go to empty slots from the right side
                    } else /*if (gBitFlags[cursorPoint - 1] & gSaveContext.inventory.equipment)*/ {
                        pauseCtx->cursorPoint[PAUSE_EQUIP] = cursorPoint;
                        pauseCtx->cursorX[PAUSE_EQUIP] = cursorX;
                        pauseCtx->cursorY[PAUSE_EQUIP] = cursorY;
        ...
        if (pauseCtx->cursorX[PAUSE_EQUIP] == 0) {
            ...
            if (LINK_AGE_IN_YEARS == YEARS_CHILD) {
                if ((pauseCtx->cursorY[PAUSE_EQUIP] == 0) && (CUR_UPG_VALUE(UPG_BULLET_BAG) != 0)) {
                    // Spaceworld - disable the bullet bag as child
                    cursorItem = oldCursorPoint; // ITEM_BULLET_BAG_30 + CUR_UPG_VALUE(UPG_BULLET_BAG) - 1;
                } else {
        ...
        if ((pauseCtx->cursorSpecialPos == 0) && (cursorItem != PAUSE_ITEM_NONE) && (pauseCtx->state == 6) &&
            ...
            if ((gEquipAgeReqs[pauseCtx->cursorY[PAUSE_EQUIP]][pauseCtx->cursorX[PAUSE_EQUIP]] == 9) ||
                ...
                // Spaceworld - Allow the cursor to go to empty slots
                // Dont allow equiping empty slots
    ...
    for (rowStart = 0, j = 0, temp = 0, i = 0; i < 4; i++, rowStart += 4, j += 16) {
        ...
        if (LINK_IS_CHILD) {
            ...
            // Spaceworld- dont draw bullet bag as child
            if ((point != 0) && (CUR_UPG_VALUE(sChildUpgrades[i]) != 0) && (i != 0)) {
                KaleidoScope_DrawQuadTextureRGBA32(globalCtx->state.gfxCtx,
                                                   gItemIcons[sChildUpgradeItemBases[i] + point - 1], 32, 32, 0);
            }
            // Spaceworld - always show '99' wallet as child
            if (i == 3) {
                KaleidoScope_DrawQuadTextureRGBA32(globalCtx->state.gfxCtx, gAdultsWalletIconTex, 32, 32, 0);
            }
        ...
        for (k = 0, bit = rowStart, point = 4; k < 3; k++, point += 4, temp++, bit++) {
            // Only draw Biggoron/Giant's Knife texture if Link actually has it (because some weird bug we had?)
            if (((u32)i == 0) && (k == 2) && (gSaveContext.bgsFlag != 0)) {
                if (gActuallyHasGiantsKnife) {
    ...
    func_800949A8(globalCtx->state.gfxCtx);
    KaleidoScope_DrawEquipmentImage(globalCtx, sPlayerRenderTex[gSaveContext.linkAge], 72,
                                    sPlayerRenderTexHeight[gSaveContext.linkAge]);
