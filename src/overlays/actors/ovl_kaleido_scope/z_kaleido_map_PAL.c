...
MovementDirection KaleidoScope_GetCursorDirection(s32 inputX, s32 inputY) {
    if (inputX > 30) {
        return MOVE_RIGHT;
    } else if (inputX < -30) {
        return MOVE_LEFT;
...
#define MAP_GO_TO_LEFT_PAGE 200
#define MAP_GO_TO_RIGHT_PAGE 300

// #define USE_EVIL_GODDESS

s32 KaleidoScope_GetNextWorldMapPoint(s32 curPoint, s32 inputX, s32 inputY) {
    // used for determining where to move the cursor to next (left, right, up, down)
    static const GoToNext cursorMoveNext[] = {
        /* MAP_SLOT_LAKE_HYLIA */ {
            MAP_SLOT_GERUDO_VALLEY,
            MAP_SLOT_HYRULE_FIELD,
            MAP_SLOT_LAKE_HYLIA,
            MAP_SLOT_LAKE_HYLIA,
        },
        /* MAP_SLOT_HYRULE_CASTLE */
        {
            MAP_SLOT_MARKET,
            MAP_SLOT_KAKARIKO,
            MAP_SLOT_HYRULE_CASTLE,
    ...
    MovementDirection dir = KaleidoScope_GetCursorDirection(inputX, inputY);

    if (dir == MOVE_LEFT) {
        return cursorMoveNext[curPoint].left;
    } else if (dir == MOVE_RIGHT) {
        return cursorMoveNext[curPoint].right;
    } else if (dir == MOVE_UP) {
        return cursorMoveNext[curPoint].up;
    } else if (dir == MOVE_DOWN) {
        return cursorMoveNext[curPoint].down;
    }

    return curPoint;
}

void KaleidoScope_HandleWorldMapCursor(GlobalContext* globalCtx) {
    PauseContext* pauseCtx = &globalCtx->pauseCtx;
    s32 cursorMoved = false;

#ifdef USE_EVIL_GODDESS
    const s16 furthestLeft = MAP_SLOT_EVIL_GODDESS;
#else
    const s16 furthestLeft = MAP_SLOT_GERUDO_VALLEY;
#endif
    const s16 furthestRight = MAP_SLOT_ZORAS_FOUNTAIN;

    if ((pauseCtx->state == 6) && (pauseCtx->unk_1E4 == 0) && (pauseCtx->pageIndex == PAUSE_MAP)) {
        if (((pauseCtx->cursorSpecialPos == PAUSE_CURSOR_PAGE_LEFT) && (pauseCtx->stickRelX > 30)) ||
            ((pauseCtx->cursorSpecialPos == PAUSE_CURSOR_PAGE_RIGHT) && (pauseCtx->stickRelX < -30))) {
            if (pauseCtx->cursorSpecialPos == PAUSE_CURSOR_PAGE_LEFT) {
                gWorldMapCursor = furthestLeft;
            } else {
                gWorldMapCursor = furthestRight;
            }
            ...
            s32 newSlot = KaleidoScope_GetNextWorldMapPoint(gWorldMapCursor, pauseCtx->stickRelX, pauseCtx->stickRelY);

            if (newSlot == MAP_GO_TO_LEFT_PAGE) {
                // gWorldMapCursor = furthestLeft;
                // Move to left side
                KaleidoScope_MoveCursorToSpecialPos(globalCtx, PAUSE_CURSOR_PAGE_LEFT);
            } else if (newSlot == MAP_GO_TO_RIGHT_PAGE) {
                // gWorldMapCursor = furthestRight;
                // Move to right side
                KaleidoScope_MoveCursorToSpecialPos(globalCtx, PAUSE_CURSOR_PAGE_RIGHT);
            } else {
                if (newSlot != gWorldMapCursor) {
                    cursorMoved = true;
                }
                gWorldMapCursor = newSlot;
            }
        }
    }
...
void KaleidoScope_DrawWorldMapPicture(GraphicsContext* gfxCtx, PauseContext* pauseCtx) {
    s32 i, j, t;
...
Gfx* KaleidoScope_DrawWorldMapCursor(GlobalContext* globalCtx, Gfx* dListHead) {
...
#endif
    static Vtx hyliaBaseVtx[] = {
        VTX(-46, -36, 0, -16, -16, 255, 255, 255, 255), VTX(-46, -51, 0, -16, 496, 255, 255, 255, 255),
        VTX(-32, -51, 0, 496, 496, 255, 255, 255, 255), VTX(-32, -36, 0, 496, -16, 255, 255, 255, 255),
 0, 496, -16, 255, 255, 255, 255),
    };
    static Vtx graveBaseVtx[] = {
        VTX(21, 24, 0, -16, -16, 255, 255, 255, 255), VTX(21, 9, 0, -16, 496, 255, 255, 255, 255),
        VTX(35, 9, 0, 496, 496, 255, 255, 255, 255),  VTX(35, 24, 0, 496, -16, 255, 255, 255, 255),
        VTX(21, 4, 0, -16, -16, 255, 255, 255, 255),  VTX(35, 4, 0, -16, 496, 255, 255, 255, 255),
        VTX(35, 18, 0, 496, 496, 255, 255, 255, 255), VTX(21, 18, 0, 496, -16, 255, 255, 255, 255),
        VTX(41, 4, 0, -16, -16, 255, 255, 255, 255),  VTX(41, 18, 0, -16, 496, 255, 255, 255, 255),
        VTX(26, 18, 0, 496, 496, 255, 255, 255, 255), VTX(26, 4, 0, 496, -16, 255, 255, 255, 255),
        VTX(41, 24, 0, -16, -16, 255, 255, 255, 255), VTX(26, 24, 0, -16, 496, 255, 255, 255, 255),
        VTX(26, 9, 0, 496, 496, 255, 255, 255, 255),  VTX(41, 9, 0, 496, -16, 255, 255, 255, 255),
    };
    static Vtx marketBaseVtx[] = {
        VTX(-23, 17, 0, -16, -16, 255, 255, 255, 255), VTX(-23, 3, 0, -16, 496, 255, 255, 255, 255),
        VTX(-9, 3, 0, 496, 496, 255, 255, 255, 255),   VTX(-9, 17, 0, 496, -16, 255, 255, 255, 255),
        VTX(-23, -3, 0, -16, -16, 255, 255, 255, 255), VTX(-9, -3, 0, -16, 496, 255, 255, 255, 255),
        VTX(-9, 11, 0, 496, 496, 255, 255, 255, 255),  VTX(-23, 11, 0, 496, -16, 255, 255, 255, 255),
        VTX(-3, -3, 0, -16, -16, 255, 255, 255, 255),  VTX(-3, 11, 0, -16, 496, 255, 255, 255, 255),
        VTX(-18, 11, 0, 496, 496, 255, 255, 255, 255), VTX(-18, -3, 0, 496, -16, 255, 255, 255, 255),
        VTX(-3, 17, 0, -16, -16, 255, 255, 255, 255),  VTX(-18, 17, 0, -16, 496, 255, 255, 255, 255),
        VTX(-18, 3, 0, 496, 496, 255, 255, 255, 255),  VTX(-3, 3, 0, 496, -16, 255, 255, 255, 255),
    };
    static Vtx* vertexLists[] = {
        /* MAP_SLOT_LAKE_HYLIA */ hyliaBaseVtx,
        /* MAP_SLOT_HYRULE_CASTLE */ castleBaseVtx,
        /* MAP_SLOT_HYRULE_FIELD */ fieldBaseVtx,
        /* MAP_SLOT_KOKIRI */ forestBaseVtx,
        /* MAP_SLOT_KAKARIKO */ kakarikoBaseVtx,
        /* MAP_SLOT_DEATH_MOUNTAIN */ deathMountainBaseVtx,
        /* MAP_SLOT_ZORAS_RIVER */ riverBaseVtx,
        /* MAP_SLOT_ZORAS_FOUNTAIN */ fountainBaseVtx,
        /* MAP_SLOT_GERUDO_VALLEY */ valleyBaseVtx,
        /* MAP_SLOT_EVIL_GODDESS */ evilGoddessBaseVtx,
        /* MAP_SLOT_GRAVEYARD */ graveBaseVtx,
        /* MAP_SLOT_MARKET */ marketBaseVtx,
    };
    ...
    vertices = Graph_Alloc(globalCtx->state.gfxCtx, sizeof(hyliaBaseVtx));
    bcopy(vertexLists[gWorldMapCursor], vertices, sizeof(hyliaBaseVtx));
    vertexCount = ARRAY_COUNT(hyliaBaseVtx);

    // Set vertex data
    for (i = 0; i < vertexCount; i++) {
        // Set the Y coordinate of each vertex
        SET_VTX_Y(vertices[i], GET_VTX_Y(vertices[i]) + pauseCtx->offsetY);
...
void KaleidoScope_DrawWorldMap(GlobalContext* globalCtx, GraphicsContext* gfxCtx) {
    ...
    KaleidoScope_HandleWorldMapCursor(globalCtx);
    KaleidoScope_DrawWorldMapPicture(gfxCtx, pauseCtx);
}
