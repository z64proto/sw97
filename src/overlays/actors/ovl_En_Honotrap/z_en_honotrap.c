...
static u8 flameFrame = 0;

void EnHonotrap_DrawFlame(Actor* thisx, GlobalContext* globalCtx) {
    s32 pad;
    EnHonotrap* this = THIS;
    Gfx* dList;
    s8 objectFireIndex = Object_GetIndex(&globalCtx->objectCtx, OBJECT_FIRE);

    OPEN_DISPS(globalCtx->state.gfxCtx);

    func_80093D84(globalCtx->state.gfxCtx);

    if (objectFireIndex >= 0) {

        dList = 0x06000040; // Hardcoded address for now

        gSegments[6] = VIRTUAL_TO_PHYSICAL(globalCtx->objectCtx.status[objectFireIndex].segment);
        gSPSegment(POLY_XLU_DISP++, 0x06, globalCtx->objectCtx.status[objectFireIndex].segment);
        ...
        gSPSegment(POLY_XLU_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(sFlameTextures[flameFrame]));
        gDPSetPrimColor(POLY_XLU_DISP++, 0x80, 0x80, 255, 200, 0, 255);
        gDPSetEnvColor(POLY_XLU_DISP++, 255, 0, 0, 0);
    } else {
        // default to normal flame if object_fire isnt loaded
        dList = gEffFire1DL;
