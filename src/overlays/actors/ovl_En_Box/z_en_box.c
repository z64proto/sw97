...
if clear, chest moves ( Actor_MoveForwardXZXZXZXZ) (falls, likely)
...
static AnimationHeader* D_809CA800[4] = {
...
void EnBox_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    this->type = thisx->params >> 11 & 0x1F;
    ...
    // helps the compiler store globalCtx2 into s1
...
void EnBox_AppearInit(EnBox* this, GlobalContext* globalCtx) {
    if (func_8005B198() == this->dyna.actor.category || this->unk_1A8 != 0) {
        ...
        // No treasure chest appearing sfx in SW
...
// Best guess, but could likely be wrong.
u8 spaceWorldGetItemTable[0x40] = {
    /* 0x00 */ GI_NONE,
    /* 0x01 */ GI_MAP,
    /* 0x02 */ GI_COMPASS,
    /* 0x03 */ GI_HEART_PIECE,
    ...
    if (this->unk_1F4 != 0) { // unk_1F4 is modified by player code
        ...
        if (sp4C.z > -50.0f && sp4C.z < 0.0f && fabsf(sp4C.y) < 10.0f && fabsf(sp4C.x) < 20.0f &&
            ...
            func_8002F554(&this->dyna.actor, globalCtx,
                          0 - spaceWorldGetItemTable[(this->dyna.actor.params >> 5 & 0x3F)]);
        }
...
void EnBox_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((this->dyna.actor.params >> 5 & 0x3F) == 0x7C && this->actionFunc == EnBox_Open &&
    ...
    */
    if ((this->alpha == 255 && !(this->type == ENBOX_TYPE_4 || this->type == ENBOX_TYPE_6)) ||
        ...
        switch (this->type) {
            ...
            case ENBOX_TYPE_ROOM_CLEAR_SMALL:
            case ENBOX_TYPE_SWITCH_FLAG_FALL_SMALL:
                gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(gTreasureChestSideAndTopTex));
                gSPSegment(POLY_OPA_DISP++, 0x0A, SEGMENTED_TO_VIRTUAL(gTreasureChestFrontTex));
            default:
                gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(gMMChestNormalTopTex));
                gSPSegment(POLY_OPA_DISP++, 0x0A, SEGMENTED_TO_VIRTUAL(gMMChestNormalFrontTex));
        }
        ...
        switch (this->type) {
            ...
            case ENBOX_TYPE_ROOM_CLEAR_SMALL:
            case ENBOX_TYPE_SWITCH_FLAG_FALL_SMALL:
                gSPSegment(POLY_XLU_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(gTreasureChestSideAndTopTex));
                gSPSegment(POLY_XLU_DISP++, 0x0A, SEGMENTED_TO_VIRTUAL(gTreasureChestFrontTex));
            default:
                gSPSegment(POLY_XLU_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(gMMChestNormalTopTex));
                gSPSegment(POLY_XLU_DISP++, 0x0A, SEGMENTED_TO_VIRTUAL(gMMChestNormalFrontTex));
        }
