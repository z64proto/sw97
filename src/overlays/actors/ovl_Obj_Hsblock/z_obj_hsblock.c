...
CollisionHeader* D_80B940DC[] = { &gHsBlockCol_000730, &gHsBlockCol_000730, &gHsBlockCol_000578 };

static Color_RGB8 sFireTempleColor[] = { { 176, 75, 22 }, { 95, 30, 12 } };

static Gfx* sDLists[] = { gHsBlockDL_000210, gHsBlockDL_000210, gHsBlockDL_000470 };
...
void ObjHsblock_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (globalCtx->sceneNum == SCENE_FIRE_TEMPLE) {
        if (globalCtx->roomCtx.curRoom.num == 8) {
            color = &sFireTempleColor[1];
        } else {
            color = &sFireTempleColor[0];
        }
    } else {
