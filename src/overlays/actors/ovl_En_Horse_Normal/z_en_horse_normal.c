...
static EnHorseNormalUnkStruct1 D_80A6D470[] = {
    ...
    */
    { -6619, -393, 6919, 0x07 },  { -5193, 124, 5433, 0x07 },   { -2970, 2, 4537, 0x07 },
    { -2949, -35, 4527, 0x07 },   { -1907, -47, 2978, 0x07 },   { 2488, 294, 3628, 0x07 },
    { 3089, 378, 4713, 0x07 },    { 1614, -261, 7596, 0x07 },   { 754, -187, 9295, 0x07 },
    { -1682, -500, 12578, 0x07 }, { -3288, -500, 13013, 0x07 }, { -5142, -417, 11630, 0x07 },
    { -5794, -473, 9573, 0x07 },  { -6765, -500, 8364, 0x07 },
};

// Unused
static EnHorseNormalUnkStruct2 D_80A6D4B8 = { ARRAY_COUNT(D_80A6D470), D_80A6D470 };

// spaceworld waypoints for old archery
static EnHorseNormalUnkStruct1 waypointPos2[] = {
    { 8386, 0, 980, 0x07 },    { 7236, 0, 689, 0x07 },   { 6200, 0, 215, 0x07 },  { -6375, 0, 209, 0x07 },
    { -7189, 0, 1138, 0x07 },  { -8128, 0, 1239, 0x07 }, { -9033, 0, 34, 0x07 },  { -8671, 0, -922, 0x07 },
    { -7472, 0, -1145, 0x07 }, { -6427, 0, -186, 0x07 }, { 6536, 0, -196, 0x07 }, { 7311, 0, -951, 0x07 },
    { 8550, 0, -1069, 0x07 },  { 9215, 0, -40, 0x07 },
};

static EnHorseNormalUnkStruct2 waypointPath2 = { ARRAY_COUNT(waypointPos2), waypointPos2 };

// unused spaceworld waypoints for gerudo valley
static EnHorseNormalUnkStruct1 waypointPos3[] = {
    { 8084, 32, -3230, 0x07 }, { 6890, 238, -3648, 0x07 }, { 5515, 10, -3698, 0x07 },  { 4881, 9, -3698, 0x07 },
    { 4280, 86, -3724, 0x07 }, { 3600, 203, -4019, 0x07 }, { 2961, 253, -4047, 0x07 }, { 3527, 232, -3719, 0x07 },
    { 4879, 7, -3700, 0x07 },  { 5542, -2, -3704, 0x07 },  { 6608, 204, -3724, 0x07 }, { 7372, 203, -3722, 0x07 },
};

static EnHorseNormalUnkStruct2 waypointPath3 = { ARRAY_COUNT(waypointPos3), waypointPos3 };
...
void EnHorseNormal_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    switch (globalCtx->sceneNum) {
        case SCENE_HYRULE_FIELD:
            this->actor.params = 0x0010;
            this->pathway = &D_80A6D4B8;
            this->actor.world.pos.x = D_80A6D470[0].pos.x;
            this->actor.world.pos.y = D_80A6D470[0].pos.y;
            this->actor.world.pos.z = D_80A6D470[0].pos.z;
            break;
        case SCENE_OLD_ARCHERY:
            this->actor.params = 0x0010;
            this->pathway = &waypointPath2;
            this->actor.world.pos.x = waypointPos2[0].pos.x;
            this->actor.world.pos.y = waypointPos2[0].pos.y;
            this->actor.world.pos.z = waypointPos2[0].pos.z;
            break;
    }
...
void EnHorseNormal_FollowPath(EnHorseNormal* this, GlobalContext* globalCtx) {
    EnHorseNormalUnkStruct2* path = this->pathway;
    EnHorseNormalUnkStruct1* pointPos = path->items;
    f32 dx;
    f32 dz;
    s32 pad;

    pointPos += this->waypoint;
    this->actor.speedXZ = pointPos->unk_06;
    dx = pointPos->pos.x - this->actor.world.pos.x;
    dz = pointPos->pos.z - this->actor.world.pos.z;
    Math_SmoothStepToS(&this->actor.world.rot.y, Math_FAtan2F(dx, dz) * (0x8000 / M_PI), 0xA, 0x7D0, 1);
    this->actor.shape.rot.y = this->actor.world.rot.y;
    if (SQ(dx) + SQ(dz) < 600.0f) {
        this->waypoint++;
        if (this->waypoint >= path->len) {
            this->waypoint = 0;
        }
    }
