...
#include "vt.h"
#include "objects/object_human/zelda_tg2_zelda_tg2.h"
...
void EnGirlA_DrawHuman(Actor* thisx, GlobalContext* globalCtx) {
    EnGirlA* this = THIS;
...
void EnGirlA_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->params == -1) { // OBJECT_HUMAN
        thisx->objBankIndex = Object_GetIndex(&globalCtx->objectCtx, OBJECT_HUMAN);
        SkelAnime_Init(globalCtx, &this->skelAnime, &testgirl2_wait_mdl_info, &testgirl2_wait_soft_anim_tbl_info,
                       this->jointTable, this->morphTable, 42);
        thisx->draw = EnGirlA_DrawHuman;
        Actor_ChangeCategory(globalCtx, &globalCtx->actorCtx, thisx, ACTORCAT_NPC);
...
void EnGirlA_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->params == 4 || thisx->params == 5) {
        Actor_RequestToTalkInRange(&this->actor, globalCtx, 100.0f); // allow talking
    }
}
