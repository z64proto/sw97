...
#include "z_en_fish_gfx.c"
extern Gfx sFishIShopDL[];

void EnFish_IShopDraw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    gSPDisplayList(POLY_XLU_DISP++, sFishIShopDL);
...
void EnFish_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (globalCtx->sceneNum == SCENE_I_SHOP) {
        this->actor.update = Actor_Noop;
        this->actor.draw = EnFish_IShopDraw;
        Actor_SetScale(thisx, 0.30f);
        this->actor.world.pos.y = 37.0f;
        return;
    }
