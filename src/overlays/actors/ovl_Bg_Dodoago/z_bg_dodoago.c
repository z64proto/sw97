...
void BgDodoago_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (Flags_GetSwitch(globalCtx, (this->dyna.actor.params & 0x3F))) {
        ...
        Flags_SetSwitch(globalCtx, 0x10);
        this->flag = true;
        return;
    } else {
...
void func_8087227C(BgDodoago* this, GlobalContext* globalCtx) {
    if (!this->flag && DECR(this->timer) == 0) {
        Flags_SetSwitch(globalCtx, 0x10); // hardcoded hack to unlock the beta DC boss door
    }
}
...
void BgDodoago_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Actor_GetProjectionPos(globalCtx, &this->dyna.actor, &sp2A, &sp28);

    if ((this->actionFunc == func_8087227C) || (sp2A >= 0) && (sp2A <= 320) && (sp28 >= 0) && (sp28 <= 240)) {
