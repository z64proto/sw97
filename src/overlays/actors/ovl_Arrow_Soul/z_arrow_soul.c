/*
 * File: z_arrow_soul.c
 * Overlay: ovl_Arrow_Soul
 * Description: Soul Arrow. Spawned as a child of a normal arrow.
 */

#include "z_arrow_soul.h"
...
void ArrowSoul_Init(Actor* thisx, GlobalContext* globalCtx);
void ArrowSoul_Destroy(Actor* thisx, GlobalContext* globalCtx);
void ArrowSoul_Update(Actor* thisx, GlobalContext* globalCtx);
void ArrowSoul_Draw(Actor* thisx, GlobalContext* globalCtx);

void ArrowSoul_Charge(ArrowSoul* this, GlobalContext* globalCtx);
void ArrowSoul_Fly(ArrowSoul* this, GlobalContext* globalCtx);
void ArrowSoul_Hit(ArrowSoul* this, GlobalContext* globalCtx);

#include "z_arrow_soul_gfx.c"

const ActorInit Arrow_Soul_InitVars = {
    ACTOR_ARROW_SOUL,
    ACTORCAT_ITEMACTION,
    FLAGS,
    OBJECT_GAMEPLAY_KEEP,
    sizeof(ArrowSoul),
    (ActorFunc)ArrowSoul_Init,
    (ActorFunc)ArrowSoul_Destroy,
    (ActorFunc)ArrowSoul_Update,
    (ActorFunc)ArrowSoul_Draw,
};
...
void ArrowSoul_SetupAction(ArrowSoul* this, ArrowSoulActionFunc actionFunc) {
...
void ArrowSoul_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    LOG_STRING("消滅", "../z_arrow_soul.c", 415);
}
...
void ArrowSoul_UnkFunc(Vec3f* unkPos, Vec3f* soulPos, f32 scale) {
...
void ArrowSoul_Fly(ArrowSoul* this, GlobalContext* globalCtx) {
    ...
    ArrowSoul_UnkFunc(&this->unkPos, &this->actor.world.pos, 0.05f);
...
void ArrowSoul_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((arrow != NULL) && (arrow->actor.update != NULL) && (this->timer < 255)) {
        ...
        gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_arrow_soul.c", 660),
