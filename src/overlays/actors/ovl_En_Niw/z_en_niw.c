...
void EnNiw_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    this->circleTargetYaw = this->actor.yawTowardsPlayer + 0x7FFF; // next follow starting point
    ...
    SkelAnime_InitFlex(globalCtx, &this->skelAnime, &gNiwSkel, &gNiwWaitAnim, this->jointTable, this->morphTable, 16);
...
void EnNiw_ResetAction(EnNiw* this, GlobalContext* globalCtx) {
    Animation_Change(&this->skelAnime, &gNiwWaitAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gNiwWaitAnim), ANIMMODE_LOOP,
                     -10.0f);
...
void func_80AB6570(EnNiw* this, GlobalContext* globalCtx) {
    Vec3f circlePath;        // active path
    Vec3f simulPathPositive; // simulated next path / facing path
    Vec3f simulPathNegative; // simulated next path / facing path
    s32 pad[2];
    f32 posY = Rand_CenteredFloat(100.0f);
    f32 posZ = Rand_CenteredFloat(100.0f);
    s16 tmp;
    Player* player = PLAYER;
    f32 circleradius;  // calculated by some stuff
    s16 yawToNegative; // yaw difference to negative simulated target
    s16 yawToPositive; // yaw difference to positive simulated target
    ...
    tmp = 0;
    if (this->actor.xyzDistToPlayerSq > 64000.0f) {
        this->bob = 0; // no bob
        if (this->timer5 != 0) {
        ...
        this->circleTargetYaw =
            this->actor.yawTowardsPlayer + 0x7FFF; // next follow starting point (might be pointless now)
        if (this->timer4 != 0) {                   // running around normally
            ...
            Math_SmoothStepToS(&this->actor.world.rot.y, Math_FAtan2F(posY, posZ) * (0x8000 / M_PI), 3, this->unk_300,
                               0);
            Math_ApproachF(&this->unk_300, 10000.0f, 1.0f, 1000.0f);
        }
    } else { // walk around Link in a circle

        if (this->actor.xzDistToPlayer < MAXCIRCLERADIUS) { // decide path radius
            if (this->actor.xzDistToPlayer < MINCIRCLERADIUS) {
                circleradius = MINCIRCLERADIUS;
            } else {
                circleradius = this->actor.xzDistToPlayer;
            }
        } else {
            circleradius = MAXCIRCLERADIUS;
        }

        // generate new path and simulations
        circlePath.x = (player->actor.world.pos.x +
                        Math_SinS(this->circleTargetYaw) * circleradius); // float is radius around Link
        circlePath.z = (player->actor.world.pos.z +
                        Math_CosS(this->circleTargetYaw) * circleradius); // float is radius around Link
        simulPathPositive.x = (player->actor.world.pos.x +
                               Math_SinS(this->circleTargetYaw + ((0x4000 / circleradius) * 5)) * circleradius);
        simulPathNegative.x = (player->actor.world.pos.x +
                               Math_SinS(this->circleTargetYaw - ((0x4000 / circleradius) * 5)) * circleradius);
        circlePath.y = simulPathPositive.y = simulPathNegative.y = this->actor.world.pos.y;
        simulPathPositive.z = (player->actor.world.pos.z +
                               Math_CosS(this->circleTargetYaw + ((0x4000 / circleradius) * 5)) * circleradius);
        simulPathNegative.z = (player->actor.world.pos.z +
                               Math_CosS(this->circleTargetYaw - ((0x4000 / circleradius) * 5)) * circleradius);

        yawToPositive = Actor_WorldYawTowardPoint(&this->actor, &simulPathPositive) - this->actor.world.rot.y;
        yawToNegative = Actor_WorldYawTowardPoint(&this->actor, &simulPathNegative) - this->actor.world.rot.y;

        if (Actor_WorldDistXZToPoint(&this->actor, &circlePath) <
            20.0f) { // choose next logical direction for path point
            if (ABS(yawToPositive) < ABS(yawToNegative)) {
                this->circleTargetYaw += (0x4000 / circleradius); // -= distance between each path point
                Math_SmoothStepToS(&this->actor.world.rot.y,
                                   Actor_WorldYawTowardPoint(&this->actor, &simulPathPositive), 1, 2000, 0x0);
            } else {
                this->circleTargetYaw -= (0x4000 / circleradius); // -= distance between each path point
                Math_SmoothStepToS(&this->actor.world.rot.y,
                                   Actor_WorldYawTowardPoint(&this->actor, &simulPathNegative), 1, 2000, 0x0);
            }
        } else {
            Math_SmoothStepToS(&this->actor.world.rot.y, Actor_WorldYawTowardPoint(&this->actor, &circlePath), 1, 2000,
                               0x0);
        }

        if (Actor_WorldDistXZToPoint(&this->actor, &circlePath) > 40.0f) {
            this->actor.speedXZ = 1.5;
            this->circleTargetYaw = this->actor.yawTowardsPlayer + 0x7FFF;
        } else {
            this->actor.speedXZ = 0;
            Math_SmoothStepToF(&this->actor.world.pos.x, circlePath.x, 1.0f, 1.5,
                               0); // switch to precision option for circling when in range
            Math_SmoothStepToF(&this->actor.world.pos.z, circlePath.z, 1.0f, 1.5, 0);
        }

        if (this->bob <= 0) { // admittedly this is a little lazy but it works
            this->bobamount = BOBSPEED;
        }
        if (this->bob > BOBHEIGHT) {
            this->bobamount = -BOBSPEED;
        }

        this->bob += this->bobamount;
    }
...
s32 EnNiw_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    ...
    Player* player = PLAYER;
    s16 headToPlayer = this->actor.shape.rot.y - this->actor.yawTowardsPlayer;
    ...
    if (limbIndex == 15) {                              // head
        if (this->actor.xyzDistToPlayerSq > 64000.0f) { // out of range, do normal thing
            rot->y += (s16)this->unk_2E0;
            this->headTarget.x = this->headTarget.y = this->headTarget.z = 0;
        } else {                                                     // follow link
            if ((headToPlayer < 22900) && (headToPlayer > -22900)) { // their heads can turn a lot...
                Math_SmoothStepToS(&this->headTarget.y, rot->y + headToPlayer, 1, 4200, 0x0);
                rot->y += this->headTarget.y;
            } else { // can't turn to look at you
                Math_SmoothStepToS(&this->headTarget.y, 0, 1, 4200, 0x0);
                rot->y += this->headTarget.y;
            }
        }
    }
...
void EnNiw_FeatherDraw(EnNiw* this, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->feathers); i++, feather++) {
        if (feather->type == 1) {
            if (!flag) {
                gSPDisplayList(POLY_XLU_DISP++, gNiw_HaneDispDL);
                flag++;
            }
            ...
            gSPDisplayList(POLY_XLU_DISP++, gNiw_HaneDL);
        }
    }
