    ...
    if (Math_SmoothStepToF(&this->dyna.actor.world.pos.y, (this->dyna.actor.home.pos.y - 200.0f) - 20.0f, 0.075f,
        ...
        if (globalCtx->state.frames & 1) {
            ...
            if (globalCtx->state.frames & 2) {
                sp5C.x += 210.0f + Rand_ZeroOne() * 230.0f;
                sp50.x += 210.0f + Rand_ZeroOne() * 230.0f;
            } else {
                sp5C.x += 330.0f + Rand_ZeroOne() * 240.0f;
                sp50.x += 330.0f + Rand_ZeroOne() * 240.0f;
            }

            sp5C.z += 80.0f + Rand_ZeroOne() * 10.0f;
            sp50.z -= 80.0f + Rand_ZeroOne() * 10.0f;
            ...
            sp5C.x += 560.0f + Rand_ZeroOne() * 5.0f;
            sp5C.z += (Rand_ZeroOne() - 0.5f) * 160.0f;
