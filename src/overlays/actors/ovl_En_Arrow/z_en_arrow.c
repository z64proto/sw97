...
void EnArrow_Init(Actor* thisx, GlobalContext* globalCtx) {
    static EffectBlureInit2 blureNormal = {
        0, 4, 0, { 255, 255, 200, 255 }, { 255, 255, 255, 255 }, { 255, 255, 200, 0 }, { 255, 255, 255, 0 }, 16,
        0, 1, 0, { 255, 255, 170, 255 }, { 255, 255, 255, 0 },
    };
    ...
    static EffectBlureInit2 blureLight = {
        0, 4, 0, { 0, 255, 200, 255 },   { 0, 255, 255, 255 }, { 0, 255, 200, 0 }, { 0, 255, 255, 0 }, 16,
        0, 1, 0, { 255, 255, 255, 255 }, { 170, 170, 170, 0 },
    };
    static EffectBlureInit2 blureDark = {
        0, 4, 0, { 0, 255, 200, 255 }, { 0, 255, 255, 255 }, { 0, 255, 200, 0 }, { 0, 255, 255, 0 }, 16,
        0, 1, 0, { 0, 0, 0, 255 },     { 0, 0, 0, 0 },
    };
    static EffectBlureInit2 blureWind = {
        0, 4, 0, { 0, 255, 200, 255 },   { 0, 255, 255, 255 }, { 0, 255, 200, 0 }, { 0, 255, 255, 0 }, 16,
        0, 1, 0, { 170, 255, 255, 255 }, { 0, 255, 0, 0 },
    };
    static EffectBlureInit2 blureSoul = {
        0, 4, 0, { 0, 255, 200, 255 },   { 0, 255, 255, 255 }, { 0, 255, 200, 0 }, { 0, 255, 255, 0 }, 16,
        0, 1, 0, { 255, 255, 170, 255 }, { 255, 255, 0, 0 },
    };
    static u32 dmgFlags[] = {
        DMG_ARROW_FIRE,  DMG_ARROW_NORMAL, DMG_ARROW_NORMAL, DMG_ARROW_FIRE, DMG_ARROW_ICE,
        DMG_ARROW_LIGHT, DMG_ARROW_DARK,   DMG_ARROW_WIND,   DMG_ARROW_SOUL, DMG_SLINGSHOT,
    };
    EnArrow* this = THIS;
    ...
    if (this->actor.params <= ARROW_SEED) {
        ...
        if (this->actor.params <= ARROW_SOUL) {
        ...
        } else if (this->actor.params == ARROW_DARK) {

            Effect_Add(globalCtx, &this->effectIndex, EFFECT_BLURE2, 0, 0, &blureDark);

        } else if (this->actor.params == ARROW_WIND) {

            Effect_Add(globalCtx, &this->effectIndex, EFFECT_BLURE2, 0, 0, &blureWind);

        } else if (this->actor.params == ARROW_SOUL) {

            Effect_Add(globalCtx, &this->effectIndex, EFFECT_BLURE2, 0, 0, &blureSoul);
        }
    ...
    if (atTouched || this->touchedPoly) {
        if (this->actor.params >= ARROW_SEED) {
            ...
            if (atTouched && (this->collider.info.atHitInfo->elemType != ELEMTYPE_UNK4)) {
                ...
                // disable actor carrying, as it wasnt a thing at this time
                if (0 && (hitActor->update != NULL) && (!(this->collider.base.atFlags & AT_BOUNCED)) &&
...
void EnArrow_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((this->actor.params >= ARROW_FIRE) && (this->actor.params <= ARROW_SOUL)) {
        s16 elementalActorIds[] = { ACTOR_ARROW_FIRE, ACTOR_ARROW_ICE,  ACTOR_ARROW_LIGHT,
                                    ACTOR_ARROW_DARK, ACTOR_ARROW_WIND, ACTOR_ARROW_SOUL };
...
void func_809B4800(EnArrow* this, GlobalContext* globalCtx) {
    ...
    if (EnArrow_Fly == this->actionFunc) {
        ...
        if (this->actor.params <= ARROW_SEED) {
            addBlureVertex = this->actor.params <= ARROW_SOUL;

            if (this->hitActor == NULL) {
                addBlureVertex &=
                    Player_SetQuadWeaponCollision(globalCtx, &this->collider, &this->weaponInfo, &sp44, &sp38);
            } else {
                if (addBlureVertex) {
