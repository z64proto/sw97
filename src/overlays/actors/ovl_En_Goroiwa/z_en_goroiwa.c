...
void func_80A4BCA0(EnGoroiwa* this) {
    ...
    worldSphere->center.y = this->actor.world.pos.y + colliderHeightOffset[(this->actor.params >> 10) & 1];
...
Vec3s slopeRoomPathwayList[10] = {
    { 5020, 1370, -1515 }, { 4428, 1054, -1515 }, { 4330, 1025, -1515 }, { 4240, 935, -1515 },
    { 4120, 885, -1515 },  { 4010, 825, -1515 },  { 3805, 825, -1515 },  { 3740, 825, -1515 },
    { 3740, 50, -1515 },   { 3739, 49, -1515 }, // needed to make it explode for some reason
};

Path slopeRoomPathway = { 10, slopeRoomPathwayList };

Vec3s compassRoomPathwayList1[5] = {
    { -2400, 571, -1050 }, { -2400, 571, -800 }, { -2550, 571, -800 }, { -2550, 571, -1050 }, { -2400, 571, -1050 },
};

Path compassRoomPathway1 = { 5, compassRoomPathwayList1 };

Vec3s compassRoomPathwayList2[5] = {
    { -2200, 451, -750 }, { -2060, 451, -750 }, { -2060, 451, -1100 }, { -2200, 451, -1100 }, { -2200, 451, -750 },
};

Path compassRoomPathway2 = { 5, compassRoomPathwayList2 };

Vec3s compassRoomPathwayList3[5] = {
    { -1800, 451, -1100 }, { -1940, 451, -1100 }, { -1940, 451, -750 }, { -1800, 451, -750 }, { -1800, 451, -1100 },
};

Path compassRoomPathway3 = { 5, compassRoomPathwayList3 };

void SetupBoulderParams(EnGoroiwa* this) {

    if (this->actor.home.pos.x < -1800) {

        this->actor.params = 0x0C00;
        this->actor.home.rot.z = 0x0000;

    } else {

        this->actor.params = 0x0500;
        this->actor.home.rot.z = 0x0001;
    }
}

Path* GetBoulderPath(EnGoroiwa* this) {

    // yikes
    if (this->actor.home.pos.x < -2300) {

        return &compassRoomPathway1;

    } else if (this->actor.home.pos.x < -2100) {

        return &compassRoomPathway2;

    } else if (this->actor.home.pos.x < -1800) {

        return &compassRoomPathway3;

    } else {

        return &slopeRoomPathway;
    }
}

void func_80A4BE54(EnGoroiwa* this, GlobalContext* globalCtx) {
    Path* path = GetBoulderPath(this);
    Vec3s* nextPos = &path->points[this->nextWaypoint];
    Vec3f nextPosF;
...
void func_80A4BF28(EnGoroiwa* this, GlobalContext* globalCtx, Vec3f* arg2) {
    ...
    currentPointPos = &path->points[this->currentWaypoint];
    pointPos = &path->points[temp_t0];
...
void func_80A4C188(EnGoroiwa* this, GlobalContext* globalCtx) {
    this->endWaypoint = GetBoulderPath(this)->count - 1;
    this->currentWaypoint = 0;
    this->nextWaypoint = 1;
    this->pathDirection = 1;
}
...
void func_80A4C1C4(EnGoroiwa* this, GlobalContext* globalCtx, s32 waypoint) {
    Path* path = GetBoulderPath(this);
    Vec3s* pointPos = &path->points[waypoint];
...
s32 func_80A4C27C(EnGoroiwa* this, GlobalContext* globalCtx) {
    s32 pad;
    Path* path = GetBoulderPath(this);
    Vec3s* nextPointPos = &path->points[this->nextWaypoint];
    Vec3s* currentPointPos = &path->points[this->currentWaypoint];
...
s32 func_80A4C6C8(EnGoroiwa* this, GlobalContext* globalCtx) {
    ...
    path = GetBoulderPath(this);
    nextPointPos = &path->points[this->nextWaypoint];
...
s32 func_80A4C814(EnGoroiwa* this, GlobalContext* globalCtx) {
    ...
    Math_StepToF(&this->actor.speedXZ, mREG(12) * 0.01f, 0.3f);
    if (Math3D_Vec3fDistSq(&nextPointPosF, &this->actor.world.pos) < 25.0f) {
    ...
    result = Math_StepToF(&this->actor.world.pos.x, nextPointPosF.x, fabsf(this->actor.velocity.x)) & 1;
    result &= Math_StepToF(&this->actor.world.pos.y, nextPointPosF.y, fabsf(this->actor.velocity.y));
    result &= Math_StepToF(&this->actor.world.pos.z, nextPointPosF.z, fabsf(this->actor.velocity.z));
    return result;
}
...
void func_80A4D944(EnGoroiwa* this, GlobalContext* globalCtx) {
    ...
    if (this->actor.bgCheckFlags & 1 && this->actor.velocity.y < 0.0f) {
        if (this->unk_1D3 & 4 && (this->actor.home.rot.z & 1) == 1) {
            func_80A4C164(this);
            func_80A4BE54(this, globalCtx);
        }
        func_80A4D9DC(this);
