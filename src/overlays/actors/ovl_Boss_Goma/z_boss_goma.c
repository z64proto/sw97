...
void BossGoma_FloorStunnedSW(BossGoma* this, GlobalContext* globalCtx);
...
/**
 * Gohma is vulnerable, from being struck by deku nuts (spaceworld)
 */
void BossGoma_SetupFloorStunnedSW(BossGoma* this) {
    this->actionFunc = BossGoma_FloorStunnedSW;
}
...
void BossGoma_Encounter(BossGoma* this, GlobalContext* globalCtx) {
    ...
    switch (this->actionState) {
        case 0: // wait for the player to enter the room
            ...
            if (globalCtx->sceneNum == SCENE_UNFINISHED_GOHMA) {
                BossGoma_SetupEncounterState4(this, globalCtx);
                Actor_SpawnAsChild(&globalCtx->actorCtx, &this->actor, globalCtx, ACTOR_DOOR_SHUTTER, 0.0f, -480.0f,
                                   -820.0f, 0, 0x0000, 0, 0x180);
            } else if (fabsf(player->actor.world.pos.x - 150.0f) < 60.0f &&
                ...
                if (gSaveContext.eventChkInf[7] & 1) {
                    ...
                    // link looks up at gohma for spaceworld
                    func_8002DF54(globalCtx, &this->actor, 1);
                    this->actionState = 1;
                }
            }
        ...
        case 2: // zoom on player from room center
            ...
            // no turn around suprised animation for spaceworld
...
void BossGoma_CeilingIdle(BossGoma* this, GlobalContext* globalCtx) {
    ...
    if (this->framesUntilNextAction == 0) {
        osSyncPrintf("****************\n");
        osSyncPrintf("child1 state: %d\n", this->childrenGohmaState[0]);
        osSyncPrintf("child2 state: %d\n", this->childrenGohmaState[1]);
        osSyncPrintf("child3 state: %d\n", this->childrenGohmaState[2]);
        osSyncPrintf("****************\n\n");
    ...
    if (this->invincibilityFrames != 0) {
        ...
        if (this->eyeClosedTimer == 0 && this->actionFunc != BossGoma_CeilingSpawnGohmas &&
            ...
            if (this->actionFunc == BossGoma_CeilingMoveToCenter || this->actionFunc == BossGoma_CeilingIdle ||
                ...
                this->framesUntilNextAction = 100;

                if (acHitInfo->toucher.dmgFlags & 1) {
                    BossGoma_SetupFloorStunnedSW(this);
                } else {
                    BossGoma_SetupFloorStunned(this);
                }
