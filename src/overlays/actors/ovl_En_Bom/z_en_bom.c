...
void EnBom_Move(EnBom* this, GlobalContext* globalCtx) {
    ...
    if ((this->actor.speedXZ != 0.0f) && (this->actor.bgCheckFlags & 8)) {
        ...
        if (this->soundCount < 3) {
            ...
            // This is a hack to fix the sound repeating bug. The real problem is that
            // wall vs floor detection is strange with bombs, possibly an issue of what updatebgcheck flags that are
            // used. Since there is no time to debug the actual problem, this fix will do.
            this->soundCount++;
        }
    ...
    if (!(this->actor.bgCheckFlags & 1)) {
        ...
        if (this->actor.bgCheckFlags & 2) {
            ...
            switch (this->bounceCount) {
                case 1:
                    this->actor.velocity.y *= -0.35f;
                    break;
                case 2:
                    this->actor.velocity.y *= -0.55f;
                    break;
                case 3:
...
void EnBom_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->params == BOMB_BODY) {
        Matrix_Push();
        // Do some sort of rotation here... god knows what
        gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, __FILE__, __LINE__),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(POLY_OPA_DISP++, gBombHandleDL);
        Matrix_Pop();
