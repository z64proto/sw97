...
void EnFloormas_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    SkelAnime_Init(globalCtx, &this->skelAnime, &gWallmasterSkel, &gWallmasterWaitAnim, this->jointTable,
                   this->morphTable, 35);
...
void EnFloormas_SetupLand(EnFloormas* this) {
    Animation_Change(&this->skelAnime, &gWallmasterFJumpAnim, 1.0f, 41.0f, 42.0f, ANIMMODE_ONCE, 5.0f);
...
void EnFloormas_SetupSplit(EnFloormas* this) {
    ...
    Animation_Change(&this->skelAnime, &gWallmasterFJumpAnim, 1.0f, 41.0f,
                     Animation_GetLastFrame(&gWallmasterFJumpAnim), ANIMMODE_ONCE, 0.0f);
...
void EnFloormas_SetupSlaveJumpAtMaster(EnFloormas* this) {
    Animation_Change(&this->skelAnime, &gWallmasterFJumpAnim, 2.0f, 0.0f, 41.0f, ANIMMODE_ONCE, 0.0f);
...
void EnFloormas_SetupGrabLink(EnFloormas* this, Player* player) {
    ...
    Animation_Change(&this->skelAnime, &gWallmasterFJumpAnim, 1.0f, 36.0f, 45.0f, ANIMMODE_ONCE, -3.0f);
    this->actor.flags &= ~1;
    this->actor.speedXZ = 0.0f;
    this->actor.velocity.y = 0.0f;
    EnFloormas_MakeInvulnerable(this);
...
void EnFloormas_SetupFreeze(EnFloormas* this) {
    Animation_Change(&this->skelAnime, &gWallmasterFJumpAnim, 1.5f, 0, 20.0f, ANIMMODE_ONCE, -3.0f);
    this->actor.speedXZ = 0.0f;
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        ...
        if (this->actionTimer == 0 && isOnGround) {
            if (this->skelAnime.endFrame < 45.0f) {
                this->skelAnime.endFrame = Animation_GetLastFrame(&gWallmasterFJumpAnim);
...
void EnFloormas_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    POLY_OPA_DISP = SkelAnime_Draw(globalCtx, this->skelAnime.skeleton, this->skelAnime.jointTable,
                                   EnFloormas_OverrideLimbDraw, NULL, this, POLY_OPA_DISP);
    if (this->collider.base.colType == COLTYPE_HARD) {
...
void EnFloormas_DrawHighlighted(Actor* thisx, GlobalContext* globalCtx) {
    ...
    POLY_XLU_DISP = SkelAnime_Draw(globalCtx, this->skelAnime.skeleton, this->skelAnime.jointTable,
                                   EnFloormas_OverrideLimbDraw, NULL, this, POLY_XLU_DISP);
    if (this->collider.base.colType == COLTYPE_HARD) {
