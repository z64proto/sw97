...
struct_80B2CEE8 D_80B2CEE8[] = {
    ...
    { OBJECT_IM, OBJECT_OPENING_DEMO1, 1, 0, 0, 10, 3, &gSWImpaSkel, &D_060029CC },
    { OBJECT_ZL4, OBJECT_OPENING_DEMO1, 1, 0, 0, 10, 2, &gSWChildZeldaSkel, &D_06000450 },
    { OBJECT_GNDD, OBJECT_GNDD, 1, -6, 0, 10, 0, &gSWGanondorfSkel, &D_06002928 },
    { OBJECT_HORSE_GANON, OBJECT_HORSE_GANON, 1, 0, 2, 20, 1, &gHorseGanonSkel, &gHorseGanonRearingAnim },
    { OBJECT_GNDD, OBJECT_GNDD, 1, -6, 0, 10, 0, &gSWGanondorfSkel, &D_060005B4 },
    { OBJECT_HORSE_GANON, OBJECT_HORSE_GANON, 1, 0, 2, 20, 1, &gHorseGanonSkel, &gHorseGanonGallopingAnim },
    { OBJECT_GNDD, OBJECT_GNDD, 1, -6, 0, 10, 0, &gSWGanondorfSkel, &D_06004260 },
    { OBJECT_GNDD, OBJECT_GNDD, 1, -6, 0, 10, 0, &gSWGanondorfSkel, &D_060050A8 },
...
void func_80B2A75C(EnViewer* this, GlobalContext* globalCtx) {
    ...
    if (params == 2) {
        ...
        if (this->skin.skelAnime.animation == &gHorseGanonRearingAnim) {
            if (curFrame == 8) {
                // make horse not neigh at the start of the cutscene
                if (globalCtx->csCtx.frames > 30) {
    ...
    if (params == 3 || params == 4) {
        ...
        if (gSaveContext.sceneSetupIndex == 5) {
            ...
            // Spaceworld... don't spawn ocarina
        ...
        if (globalCtx->sceneNum == SCENE_HYRULE_FIELD) { // Hyrule Field
            switch (this->unk_1E5) {
                case 0:
...
void EnViewer_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    // Make lightning light up the environmnent in Hyrule Field intro cutscene
    globalCtx->envCtx.lightning = 1;
    *(f32*)(((u32)&D_8015FD70) + 0x8) = 0; // gLightningStrike.delayTimer = 0;
...
void func_80B2B4A8(EnViewer* this, GlobalContext* globalCtx) {
    ...
    if ((params == 3) || (params == 5) || (params == 7) || (params == 8)) {
        extern u64 gGanondorfEyes1Tex[];
        extern u64 gGanondorfEyes2Tex[];
        extern u64 gGanondorfEyes3Tex[];
        extern u64 gGanondorfMouth1Tex[];
        extern u64 gGanondorfMouth2Tex[];
        extern u64 gGanondorfMouth3Tex[];

        static void* ganondorfEyes[] = {
            gGanondorfEyes1Tex,
            gGanondorfEyes2Tex,
            gGanondorfEyes3Tex,
        };
        static void* ganondorfMouth[] = {
            gGanondorfMouth1Tex,
            gGanondorfMouth2Tex,
            gGanondorfMouth3Tex,
        };
        ...
        // Set Ganondorf's eye texture
        if (globalCtx->csCtx.frames <= 1120) {
            eyeIdx = 0;
        } else if (globalCtx->csCtx.frames == 1121) {
            eyeIdx = 1;
        } else {
            // 1122
            eyeIdx = 2;
        }
        gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(ganondorfEyes[eyeIdx]));

        // Set Ganondorf's mouth texture
        if (globalCtx->csCtx.frames <= 1140) {
            mouthIdx = 0;
        } else if (globalCtx->csCtx.frames == 1141) {
            mouthIdx = 1;
        } else {
            // 1142
            mouthIdx = 2;
        }
        gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(ganondorfMouth[mouthIdx]));
    } else if (params == 9) {
...
void func_80B2BA38(EnViewer* this, GlobalContext* globalCtx) {
    ...
    if (globalCtx->sceneNum == SCENE_HYRULE_FIELD) { // Hyrule Field
        if (globalCtx->csCtx.frames < 771) {
            gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(&gSWChildZeldaEyeInTex));
            gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(&gSWChildZeldaEyeOutTex));
        } else if (globalCtx->csCtx.frames < 772) {
            gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(&gSWChildZeldaEyeBlinkTex));
            gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(&gSWChildZeldaEyeBlinkTex));
        } else if (globalCtx->csCtx.frames < 773) {
            gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(&gSWChildZeldaEyeShutTex));
            gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(&gSWChildZeldaEyeShutTex));
        } else if (globalCtx->csCtx.frames < 791) {
            gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(&gSWChildZeldaEyeWideTex));
            gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(&gSWChildZeldaEyeWideTex));
