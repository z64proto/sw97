...
extern AnimationHeader D_06001C28; // ironknack_attackA
extern AnimationHeader D_06002538; // ironknack_attackA_end
extern AnimationHeader D_060029FC; // ironknack_attackA_hit
extern AnimationHeader D_060033C4; // ironknack_attackB
extern AnimationHeader D_06003DBC; // ironknack_attackB_end
extern AnimationHeader D_060045BC; // ironknack_back_hit
extern AnimationHeader D_0600485C; // ironknack_defense
extern AnimationHeader D_06005944; // ironknack_down
extern AnimationHeader D_06006194; // ironknack_front_hit
extern AnimationHeader D_06006734; // ironknack_run
extern AnimationHeader D_0600C114; // ironknack_stand_demo
extern AnimationHeader D_0600CD70; // ironknack_stand_up
extern AnimationHeader D_0600DD50; // ironknack_wait
extern AnimationHeader D_0600ED24; // ironknack_wait_hit
...
extern AnimationHeader D_060203D8; // naborknack_damage_demo
...
#include "objects/object_ironknack/zelda_ironknack_zelda_ironknack.c"
...
s32 func_80A745E4(EnIk* this, GlobalContext* globalCtx) {
    if (((this->unk_2FB != 0) || (this->actor.params == 0) || true) &&
        (func_800354B4(globalCtx, &this->actor, 150.0f, 0x7710, 0x7000, this->actor.shape.rot.y) != 0) &&
...
void func_80A74714(EnIk* this) {
    ...
    Animation_PlayLoop(&this->skelAnime2, &IronKnack_wait_soft_anim_tbl_info);
}

void func_80A747C0(EnIk* this, GlobalContext* globalCtx) {
    Vec3f sp24;

    if (this->skelAnime2.curFrame > 5.0f) {
...
void func_80A74E2C(EnIk* this) {
    ...
    this->unk_2F8 = 6;
    this->actor.speedXZ = 0.0f;
    Animation_Change(&this->skelAnime, &D_06001C28, 1.0f, 0.0f, frames, ANIMMODE_ONCE, -4.0f);
    EnIk_SetupAction(this, func_80A74EBC);

    Animation_PlayOnce(&this->skelAnime2, &IronKnack_hasso_soft_anim_tbl_info);
}
...
void func_80A7510C(EnIk* this, GlobalContext* globalCtx) {
    ...
    if (this->skelAnime2.animation != &IronKnack_hasso_soft_anim_tbl_info ||
        this->skelAnime2.curFrame == this->skelAnime2.endFrame || SkelAnime_Update(&this->skelAnime) ||
        (--this->unk_2F9 == 0)) {
        if (this->unk_2F8 == 8) {
            func_80A7489C(this);
        } else {
...
void func_80A751C8(EnIk* this) {
    ...
    Animation_PlayOnce(&this->skelAnime2, &IronKnack_hineri_soft_anim_tbl_info);
}
...
void func_80A75260(EnIk* this, GlobalContext* globalCtx) {
    ...
    if (SkelAnime_Update(&this->skelAnime) || this->skelAnime.curFrame > 11.0f) {
        func_80A753D0(this);
    }
}
...
void func_80A7545C(EnIk* this, GlobalContext* globalCtx) {
    if (1 || SkelAnime_Update(&this->skelAnime)) {
        func_80A7489C(this);
        func_80A745E4(this, globalCtx);
    }
}
...
void func_80A755F0(EnIk* this) {
    ...
    Animation_PlayOnce(&this->skelAnime2, &IronKnack_defense_start_soft_anim_tbl_info);
}

void func_80A7567C(EnIk* this, GlobalContext* globalCtx) {
    CollisionCheck_SetAC(globalCtx, &globalCtx->colChkCtx, &this->shieldCollider.base);
    if (SkelAnime_Update(&this->skelAnime) && this->skelAnime2.animation == &IronKnack_defense_end_soft_anim_tbl_info &&
        this->skelAnime2.curFrame == this->skelAnime2.endFrame) {
        if ((ABS((s16)(this->actor.yawTowardsPlayer - this->actor.shape.rot.y)) <= 0x6000) &&
            (this->actor.xzDistToPlayer < 125.0f) && (ABS(this->actor.yDistToPlayer) < 175.0f)) {
            if ((globalCtx->gameplayFrames & 1)) {
                func_80A74E2C(this);
            } else {
                func_80A751C8(this);
...
void func_80A75790(EnIk* this) {
    ...
    if (ABS(yawDiff) <= 0x4000) {
        ...
        Animation_PlayOnce(&this->skelAnime2, &IronKnack_maegiri_dmg_soft_anim_tbl_info);
    } else {
        Animation_Change(&this->skelAnime, &D_060045BC, 1.0f, 0.0f, Animation_GetLastFrame(&D_060045BC), ANIMMODE_ONCE,
                         -4.0f);
        this->actor.speedXZ = 6.0f;

        Animation_PlayOnce(&this->skelAnime2, &IronKnack_backgiri_dmg_soft_anim_tbl_info);
    }
    this->unk_2FE = 0;
...
void func_80A7598C(EnIk* this) {
    ...
    Animation_PlayOnce(&this->skelAnime2, &IronKnack_m_down_soft_anim_tbl_info);
}
...
void func_80A75FA0(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (SkelAnime_Update(&this->skelAnime2)) {
        if (this->skelAnime2.animation == &IronKnack_hineri_soft_anim_tbl_info) {
            // Animation_Reverse(&this->skelAnime2);
        } else if (this->skelAnime2.animation == &IronKnack_defense_start_soft_anim_tbl_info) {
            Animation_PlayOnce(&this->skelAnime2, &IronKnack_defense_end_soft_anim_tbl_info);
        } else if (this->skelAnime2.animation == &IronKnack_m_down_soft_anim_tbl_info) {
            ;
        } else {
            Animation_PlayLoop(&this->skelAnime2, &IronKnack_wait_soft_anim_tbl_info);
        }
...
s32 EnIk_OverrideLimbDrawBeta(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
                              void* thisx) {
    EnIk* this = THIS;

    func_80093D18(globalCtx->state.gfxCtx);

    if (limbIndex == 0x29) { // right hand shield
        *dList = sk_sword_model;
    }
...
void func_80A76798(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Actor_DrawHiliteReflectionOpa(&this->actor, globalCtx, 1); // setup OPA hilite in segment 0x07
    SkelAnime_DrawOpa(globalCtx, this->skelAnime2.skeleton, this->skelAnime2.jointTable, EnIk_OverrideLimbDrawBeta,
                      NULL, this);
...
void func_80A772EC(EnIk* this, GlobalContext* globalCtx) {
    ...
    SkinMatrix_Vec3fMtxFMultXYZW(&globalCtx->projectionMatrix, &this->actor.world.pos, &D_80A78FA0, &wDest);
...
void EnIk_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (((this->actor.params & 0xFF) == 0 && (gSaveContext.eventChkInf[3] & 0x1000)) ||
        ...
        SkelAnime_Init(globalCtx, &this->skelAnime2, &IronKnack_wait_mdl_info, &IronKnack_wait_soft_anim_tbl_info, NULL,
                       NULL, 0);
        Animation_PlayLoop(&this->skelAnime2, &IronKnack_wait_soft_anim_tbl_info);
