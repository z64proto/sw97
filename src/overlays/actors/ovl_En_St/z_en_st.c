...
void EnSt_TurnToPlayer(EnSt* this, GlobalContext* globalCtx);
void EnSt_InitRotation(EnSt* this, GlobalContext* globalCtx);
...
s32 EnSt_IsDoneBouncing(EnSt* this, GlobalContext* globalCtx) {
    ...
    // Changed for SW, now bounces high once and then bounces rapidly on the floor
    this->actor.velocity.y = 3.0f * (4 - this->groundBounces);
    this->groundBounces--;
...
s32 EnSt_CheckHitLink(EnSt* this, GlobalContext* globalCtx) {
    ...
    if (!hit || this->gaveDamageSpinTimer != 0) {
        return false;
    }
    // gSaveContext.rupees = this->gaveDamageSpinTimer;
    if (this->swayTimer == 0) {
...
void EnSt_SpinBack(EnSt* this, GlobalContext* globalCtx) {
    s16 yawDiff = this->actor.shape.rot.y - this->homeTarget;
    yawDiff = ABS(yawDiff);
    ...
    if (yawDiff <= 0x4000) {
        ...
        if (yawDiff >= 0x800) {
            minStep = 0x800;
        } else if (yawDiff >= 0x200) {
            minStep = 0x200;
        } else if (yawDiff >= 0x80) {
            minStep = 0x80;
        } else if (yawDiff >= 0x20) {
            minStep = 0x20;
        } else if (yawDiff < 0x20) {
            minStep = 1;
        }

        Math_SmoothStepToS(&this->actor.shape.rot.y, this->homeTarget, 4, 0x800, minStep);
    } else {
        this->actor.shape.rot.y += 0x2000;
    }

    if (this->actor.shape.rot.y == this->homeTarget) {
        this->actionFunc = EnSt_WaitOnGround;
    }

    if (this->actor.world.pos.y < (this->actor.floorHeight + this->floorHeightOffset)) {
        Math_SmoothStepToF(&this->actor.world.pos.y, this->actor.floorHeight + this->floorHeightOffset, 1.0f, 5.0f,
                           1.0f);
    }
...
void EnSt_SpinWait(EnSt* this, GlobalContext* globalCtx) {
    if (this->spinTimer < 10) {
        if ((this->spinTimer % 2) != 0) {
...
void EnSt_SpinAway(EnSt* this, GlobalContext* globalCtx) {
    s16 yawDiff;
    ...
    if (this->spinTimer == 0) {
        yawDiff = this->actor.shape.rot.y - this->awayTarget;

        if (ABS(yawDiff) <= 0x4000) {
            Math_SmoothStepToS(&this->actor.shape.rot.y, this->awayTarget, 4, 0x2000, 1);
        } else {
            this->actor.shape.rot.y += 0x2000;
        }

        if (this->actor.shape.rot.y == this->awayTarget) {
            this->actionFunc = EnSt_SpinWait;
            this->spinTimer = 40;
        }
    }
...
s32 EnSt_CheckHitFrontside(EnSt* this) {
    ...
    if (!!(acFlags & AC_HIT) == 0) {
        ...
        this->spinTimer = 12;
        func_80034EC0(&this->skelAnime, sAnimations, 3);
        this->homeTarget = this->actor.shape.rot.y;
        this->awayTarget = this->homeTarget ^ 0x8000;
        this->actionFunc = EnSt_SpinAway;
        this->actor.velocity.y = 0.0f;
        return true;
    }
}
...
void EnSt_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    this->initalYaw = this->actor.shape.rot.y;
    EnSt_SetupAction(this, EnSt_InitRotation);
}
...
void EnSt_InitRotation(EnSt* this, GlobalContext* globalCtx) {
    Player* player = PLAYER;
    s16 yawDiff = this->actor.world.rot.y - this->actor.yawTowardsPlayer;
    s16 yawRotated;
    s16 checkNearer180;
    s16 checkNearer90;

    yawRotated = this->initalYaw - this->actor.yawTowardsPlayer;
    checkNearer180 = DEGF_TO_BINANG(180) - yawRotated;
    checkNearer90 = DEGF_TO_BINANG(90) - yawRotated;

    if (ABS(yawDiff) >= ABS(DEGF_TO_BINANG(67.5))) {
        if (ABS(checkNearer180) < ABS(checkNearer90)) {
            this->actor.shape.rot.y = this->initalYaw + ABS(DEGF_TO_BINANG(180));
        } else if (BINANG_TO_DEGF(yawRotated) < 0) {
            this->actor.shape.rot.y = this->initalYaw + ABS(DEGF_TO_BINANG(90));
        } else {
            this->actor.shape.rot.y = this->initalYaw - ABS(DEGF_TO_BINANG(90));
        }
    }
...
void EnSt_TurnToPlayer(EnSt* this, GlobalContext* globalCtx) {
    ...
    if ((ABS(yawDiff) >= ABS(DEGF_TO_BINANG(67.5))) && (!this->rotateFlag) && (EnSt_IsCloseToPlayer(this, globalCtx))) {
        this->initalYaw = this->actor.shape.rot.y;
        this->rotateFlag = true;
    }
    if ((yawDiff != 0) && this->rotateFlag) {
        Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 1, DEGF_TO_BINANG(15), 1);
        yawDiff = this->actor.shape.rot.y - this->actor.yawTowardsPlayer;
        if (ABS(yawDiff) < ABS(DEGF_TO_BINANG(2))) {
            this->rotateFlag = false;
            if (yawDiff != 0) {
                this->actor.shape.rot.y = this->initalYaw - (yawDiff < 0 ? DEGF_TO_BINANG(-90) : DEGF_TO_BINANG(90));
            }
        }
    }
    this->actor.world.rot.y = this->actor.shape.rot.y;
...
void EnSt_WaitOnGround(EnSt* this, GlobalContext* globalCtx) {
    ...
    EnSt_TurnToPlayer(this, globalCtx);
    EnSt_Bob(this, globalCtx);
}
