...
#include "objects/object_oc/zelda_oc_zelda_oc.c"
#include "objects/object_oc/zelda_rock_zelda_rock.c"
...
void EnOkuta_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    thisx->params &= 0xFF;
    if (thisx->params == 0) {
        SkelAnime_Init(globalCtx, &this->skelAnime, &okuta_wait_mdl_info, &okuta_wait_soft_anim_tbl_info, NULL, NULL,
                       0);
...
void EnOkuta_ProjectileFly(EnOkuta* this, GlobalContext* globalCtx) {
    ...
    if ((this->actor.bgCheckFlags & 8) || (this->actor.bgCheckFlags & 1) || (this->collider.base.atFlags & AT_HIT) ||
        ...
        if ((player->currentShield == PLAYER_SHIELD_DEKU ||
            ...
            pos.z = this->actor.world.pos.z;
            EffectSsHahen_SpawnBurst(globalCtx, &pos, 6.0f, 0, 1, 2, 15, 7, 10, gOctorokProjectileDL_mod);
