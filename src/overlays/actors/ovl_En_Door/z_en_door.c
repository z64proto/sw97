...
static EnDoorInfo sDoorInfo[] = {
    ...
    // { SCENE_UNFINISHED_FIRE_TEMPLE, 1, OBJECT_HIDAN_OBJECTS },
    // { SCENE_WATER_TEMPLE, 2, OBJECT_MIZU_OBJECTS }, // The SW97 Water Temple does not seem to have this object loaded
...
static InitChainEntry sInitChain[] = {
    ...
    ICHAIN_F32(uncullZoneForward, 1200, ICHAIN_STOP),
};
...
static Gfx* sDekuBossDoorDLists[][2] = {
    { gDekuBossDoorLeftDL, gDekuBossDoorRightDL },
};
...
void EnDoor_Idle(EnDoor* this, GlobalContext* globalCtx) {
    ...
    if (this->playerIsOpening != 0) {
    ...
    } else if (!Player_InCsMode(globalCtx) || gSaveContext.fairyTimer != 0) { // addition for spaceworld
        ...
        if (USE_DOOR_COLLISON(globalCtx)) {
            if (fabsf(playerPosRelToDoor.y) < 100.0f && fabsf(playerPosRelToDoor.x) < 35.0f &&
                fabsf(playerPosRelToDoor.z) < 28.0f) {
                player->actor.world.pos.x += Math_SinS(this->actor.shape.rot.y) *
                                             (28.0f - fabsf(playerPosRelToDoor.z)) *
                                             ((playerPosRelToDoor.z >= 0.0f) ? 1.0f : -1.0f);
                player->actor.world.pos.z += Math_CosS(this->actor.shape.rot.y) *
                                             (28.0f - fabsf(playerPosRelToDoor.z)) *
                                             ((playerPosRelToDoor.z >= 0.0f) ? 1.0f : -1.0f);
            }
        }
...
s32 EnDoor_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 4) {
        if (globalCtx->sceneNum == SCENE_DEKU_TREE) {
            temp_a2 = sDekuBossDoorDLists[0];
        } else {
