...
void BgHidanSyoku_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    this->dyna.actor.home.pos.y += 550.0f;
}
...
void func_8088F4B8(BgHidanSyoku* this, GlobalContext* globalCtx) {
    ...
    // Spaceworld: remove the requirement for room clear
...
void func_8088F514(BgHidanSyoku* this, GlobalContext* globalCtx) {
    ...
    this->dyna.actor.world.pos.y = (cosf(this->timer * (M_PI / 140)) * 550.0f) + this->dyna.actor.home.pos.y;
    if (this->timer == 0) {
        func_8088F47C(this);
    } else {
...
void func_8088F5A0(BgHidanSyoku* this, GlobalContext* globalCtx) {
    ...
    this->dyna.actor.world.pos.y = this->dyna.actor.home.pos.y - (cosf(this->timer * (M_PI / 140)) * 550.0f);
    if (this->timer == 0) {
        func_8088F47C(this);
    } else {
        func_8002F974(&this->dyna.actor, NA_SE_EV_ELEVATOR_MOVE3 - SFX_FLAG);
