...
static ColliderJntSphElementInit sElements[1] = {
    {
        {
            ELEMTYPE_UNK0,
...
static Vec3f D_80AE1B6C;
static u8 poeRaceSpawned;

void EnPoh_Init(Actor* thisx, GlobalContext* globalCtx) {
    EnItem00* collectible;
    EnPoh* this = THIS;

    // Spawn dampe race as child actor
    if (thisx->params == 0x000A) {
        if (poeRaceSpawned) { // prevent duping
            Actor_Kill(&this->actor);
            return;
        } else {
            poeRaceSpawned = 1;
            thisx->room = -1;
            Actor_SpawnAsChild(&globalCtx->actorCtx, thisx, globalCtx, ACTOR_EN_PO_RELAY, thisx->world.pos.x,
                               thisx->world.pos.y, thisx->world.pos.z, thisx->world.rot.x, thisx->world.rot.y,
                               thisx->world.rot.z, 0x0007);
        }
    }
    this->actor.targetArrowOffset = 3200.0f;
...
void func_80ADE28C(EnPoh* this) {
    ...
    // Spaceworld -  no red flash when getting hit
...
void func_80ADEA5C(EnPoh* this, GlobalContext* globalCtx) {
    if (Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) >
        ((globalCtx->sceneNum == SCENE_SPECIAL_COURSE) ? 300.0f : 400.0f)) {
...
void func_80ADEC9C(EnPoh* this, GlobalContext* globalCtx) {
    ...
    if (this->actor.xzDistToPlayer > 280.0f) {
        EnPoh_SetupIdle(this);
               ...
               this->actor.child == NULL) { // never attack during race
...
void func_80ADF15C(EnPoh* this, GlobalContext* globalCtx) {
    ...
    if (this->unk_198 < 8) {
        ...
        // Spaceworld - drop ruppees instead of soul when killed
    ...
    if (this->unk_198 == 18) {
        ...
        // play sfx using deadsound so it keeps playing when the actor dies
        EffectSsDeadSound_SpawnStationary(globalCtx, &this->actor.projectedPos, NA_SE_EN_PO_DEAD2, true,
                                          DEADSOUND_REPEAT_MODE_OFF, 60);
    }
}
...
// Spaceworld - Custom death procedure added
/*
void EnPoh_Death(EnPoh* this, GlobalContext* globalCtx) {
    s32 objId;
...
void EnPoh_UpdateVisibility(EnPoh* this) {
    if (this->actionFunc != EnPoh_Appear && this->actionFunc != EnPoh_Disappear && this->actionFunc != func_80ADEF38 &&
        ...
        if (this->lightColor.a == 255 && this->actor.child == NULL) { // never disappear if dampe race
...
void EnPoh_UpdateLiving(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((this->colliderCyl.base.acFlags & AC_ON) && this->lightColor.a == 255 &&
        thisx->child == NULL) { // no collision during dampe race
    ...
    // Follow dampe race actor
    if (thisx->child != NULL) {
        this->actor.targetMode = 0;
        thisx->world = thisx->child->world;
    } else if (globalCtx->sceneNum == SCENE_SPECIAL_COURSE && globalCtx->roomCtx.curRoom.num == 1) {
        // Prevent race poe from wandering off after the race
        this->actor.targetMode = 2;
        this->actor.home.pos.x = 2900.0f;
        this->actor.home.pos.z = -1570.0f;
    }
}
...
void EnPoh_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfxP) {
    ...
    if (limbIndex == this->info->unk_6) {
        ...
        Lights_PointNoGlowSetInfo(&this->lightInfo, this->colliderSph.elements[0].dim.worldSphere.center.x,
...
void EnPoh_DrawRegular(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actionFunc != func_80ADF15C) {
...
// Spaceworld - Poh souls were removed
/*
void EnPoh_UpdateDead(Actor* thisx, GlobalContext* globalCtx) {
    EnPoh* this = THIS;
