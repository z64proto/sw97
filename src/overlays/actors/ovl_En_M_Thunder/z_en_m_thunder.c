...
void func_80A9F408(EnMThunder* this, GlobalContext* globalCtx) {
    ...
    if (player->stateFlags2 & 0x20000) {
        ...
        if (player->unk_858 <= 0.15f) {
            if ((player->unk_858 >= 0.1f) && (player->swordAnimation >= 0x18)) {
                // commented out to prevent magic sfx happening on the first charge frame
                // Audio_PlaySoundGeneral(NA_SE_IT_ROLLING_CUT, &player->actor.projectedPos, 4, &D_801333E0,
                // &D_801333E0,
                //                       &D_801333E8);
                // Audio_PlaySoundGeneral(NA_SE_IT_SWORD_SWING_HARD, &player->actor.projectedPos, 4, &D_801333E0,
    ...
    if (player->unk_858 > 0.15f) {
        this->unk_1C8 = 255;
        // if (this->actor.child == NULL) {
        //     Actor_SpawnAsChild(&globalCtx->actorCtx, &this->actor, globalCtx, ACTOR_EFF_DUST,
    ...
    // audio engine wrappers for charge sounds
    // disable for spaceworld
    // if (player->unk_858 > 0.85f) {
...
void EnMThunder_Draw(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    switch (this->unk_1C6) {
        case 0:
            gDPSetPrimColor(POLY_XLU_DISP++, 0, 0x80, 255, 255, 255, (u8)((u32)(this->unk_1B0 * 255) & 0xFF));
    ...
    switch (this->unk_1C7) {
        case 1:
            ...
            // change charge size. x for length, y for diameter
            Matrix_Scale(-0.6f, -1.0f, -0.4f, MTXMODE_APPLY);
            Matrix_RotateX(16384.0f, MTXMODE_APPLY);
            break;
        case 0:
            // adult
            Matrix_Translate(0.0f, 300.0f, -100.0f, MTXMODE_APPLY);
            Matrix_Scale(-1.2f, -2.3f, -0.7f, MTXMODE_APPLY);
            Matrix_RotateX(16384.0f, MTXMODE_APPLY);
            break;
        case 2:
    ...
    //     gDPSetPrimColor(POLY_XLU_DISP++, 0, 0x80, 255, 255, 255, this->unk_1C8);
    ...
    phi_f14 = (D_80AA046C[globalCtx->gameplayFrames & 7]) + 0.4f;
    gDPSetPrimColor(POLY_XLU_DISP++, 0, 0x80, 255, 255, 255, this->unk_1C8);
    gDPSetEnvColor(POLY_XLU_DISP++, 255, 255, 255, 128);
    phi_t1 = 0x14;
