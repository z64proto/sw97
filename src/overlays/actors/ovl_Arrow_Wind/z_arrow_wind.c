/*
 * File: z_arrow_wind.c
 * Overlay: ovl_Arrow_Wind
 * Description: Wind Arrow. Spawned as a child of a normal arrow.
 */
...
void ArrowWind_Init(Actor* thisx, GlobalContext* globalCtx);
void ArrowWind_Destroy(Actor* thisx, GlobalContext* globalCtx);
void ArrowWind_Update(Actor* thisx, GlobalContext* globalCtx);
void ArrowWind_Draw(Actor* thisx, GlobalContext* globalCtx);

void ArrowWind_Charge(ArrowWind* this, GlobalContext* globalCtx);
void ArrowWind_Fly(ArrowWind* this, GlobalContext* globalCtx);
void ArrowWind_Hit(ArrowWind* this, GlobalContext* globalCtx);

#include "z_arrow_wind_gfx.c"

const ActorInit Arrow_Wind_InitVars = {
    ACTOR_ARROW_WIND,
    ACTORCAT_ITEMACTION,
    FLAGS,
    OBJECT_GAMEPLAY_KEEP,
    sizeof(ArrowWind),
    (ActorFunc)ArrowWind_Init,
    (ActorFunc)ArrowWind_Destroy,
    (ActorFunc)ArrowWind_Update,
    (ActorFunc)ArrowWind_Draw,
};
...
void ArrowWind_SetupAction(ArrowWind* this, ArrowWindActionFunc actionFunc) {
...
void ArrowWind_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    LOG_STRING("消滅", "../z_arrow_wind.c", 415);
}
...
void ArrowWind_UnkFunc(Vec3f* unkPos, Vec3f* windPos, f32 scale) {
...
void ArrowWind_Fly(ArrowWind* this, GlobalContext* globalCtx) {
    ...
    ArrowWind_UnkFunc(&this->unkPos, &this->actor.world.pos, 0.05f);

    if (arrow->hitFlags & 1) {
        Audio_PlayActorSound2(&this->actor, NA_SE_IT_EXPLOSION_FRAME);
        Audio_PlayActorSound2(&this->actor, NA_SE_EN_FREEZAD_BREATH);
...
void ArrowWind_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((arrow != NULL) && (arrow->actor.update != NULL) && (this->timer < 255)) {
        ...
        gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_arrow_wind.c", 660),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(POLY_XLU_DISP++, sTextureDL);
        gSPDisplayList(POLY_XLU_DISP++,
                       Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 511 - (stateFrames * 3) % 512, 0, 64, 32, 1,
